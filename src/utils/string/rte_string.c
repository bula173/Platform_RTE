...
rte_status_t rte_string_init(rte_string_t *str, char *storage, size_t capacity)
{
    ...
    return rte_buffer_init(&str->buf, (void *)storage, capacity);
}
...
rte_status_t rte_string_clear(rte_string_t *str)
{
    ...
    return rte_buffer_clear(&str->buf);
}
...
rte_status_t rte_string_c_str(rte_string_t *str, const char **out_cstr)
{
    ...
    if (!rte_buffer_is_valid(&str->buf))
    {
        return RTE_STATUS_INVALID_PARAM;
    }
...
rte_status_t rte_string_copy_n(rte_string_t *dest, const char *src, size_t src_len)
{
    ...
    return rte_buffer_copy_in(&dest->buf, (const void *)src, src_len);
}
...
rte_status_t rte_string_copy(rte_string_t *dest, const char *src)
{
    ...
    if (!rte_buffer_is_valid(&dest->buf))
    {
        return RTE_STATUS_INVALID_PARAM;
    }
    ...
    return rte_string_copy_n(dest, src, i);
}
...
rte_status_t rte_string_concat(rte_string_t *dest, const char *src)
{
    ...
    /* Same off-by-one reasoning as rte_string_copy(): a NUL at
...
rte_status_t rte_string_compare(const rte_string_t *a, const rte_string_t *b, int32_t *out_cmp)
{
    ...
    if ((!rte_buffer_is_valid(&a->buf)) || (!rte_buffer_is_valid(&b->buf)))
    {
        return RTE_STATUS_INVALID_PARAM;
    }
                                       ...
                                       bool *out_found, size_t *out_index)
{
    ...
    if ((!rte_buffer_is_valid(&haystack->buf)) || (!rte_buffer_is_valid(&needle->buf)))
    {
        return RTE_STATUS_INVALID_PARAM;
    }
 ...
 */
static void rte_string_format_u64_digits(uint64_t value, char *out, size_t *out_len)
{
    char rev[20];
    size_t n = 0U;
    uint64_t v = value;
...
rte_status_t rte_string_from_u64(rte_string_t *dest, uint64_t value)
{
    ...
    rte_string_format_u64_digits(value, digits, &len);
    return rte_string_copy_n(dest, digits, len);
}
...
rte_status_t rte_string_from_i64(rte_string_t *dest, int64_t value)
{
    ...
    {
        buf[0] = '-';
        rte_string_format_u64_digits(magnitude, &buf[1], &len);
        return rte_string_copy_n(dest, buf, len + 1U);
    }
    rte_string_format_u64_digits(magnitude, buf, &len);
    return rte_string_copy_n(dest, buf, len);
}
...
rte_status_t rte_string_from_u32(rte_string_t *dest, uint32_t value)
{
    ...
    st = rte_cast_u32_to_u64(value, &widened);
    if (st != RTE_STATUS_OK) /* GCOVR_EXCL_START - rte_cast_u32_to_u64() can
                                * only fail for a NULL out pointer, and
                                * &widened is always a valid local address. */
    {
        return st;
    } /* GCOVR_EXCL_STOP */
    return rte_string_from_u64(dest, widened);
}
...
rte_status_t rte_string_from_i32(rte_string_t *dest, int32_t value)
{
    ...
    st = rte_cast_i32_to_i64(value, &widened);
    if (st != RTE_STATUS_OK) /* GCOVR_EXCL_START - rte_cast_i32_to_i64() can
                                * only fail for a NULL out pointer, and
                                * &widened is always a valid local address. */
    {
        return st;
    } /* GCOVR_EXCL_STOP */
    return rte_string_from_i64(dest, widened);
}
 ...
 *        rte_string_append_*() below - same append semantics as
 *        rte_string_concat() but for a known, not-NUL-terminated length.
 ...
 *         RTE_STATUS_RESOURCE_EXHAUSTED if n exceeds dest's remaining
 *         capacity (dest left unmodified).
 */
static rte_status_t rte_string_append_bytes(rte_string_t *dest, const char *src, size_t n)
{
    size_t remaining;
    char *data;
...
rte_status_t rte_string_append_u64(rte_string_t *dest, uint64_t value)
{
    ...
    rte_string_format_u64_digits(value, digits, &len);
    return rte_string_append_bytes(dest, digits, len);
}
...
rte_status_t rte_string_append_i64(rte_string_t *dest, int64_t value)
{
    ...
    if (negative)
    {
        /* Safe negation avoiding overflow for INT64_MIN (see rte_string_from_i64). */
        magnitude = (uint64_t)(-(value + 1)) + 1U;
        buf[0] = '-';
        rte_string_format_u64_digits(magnitude, &buf[1], &len);
        return rte_string_append_bytes(dest, buf, len + 1U);
    }
    magnitude = (uint64_t)value;
    rte_string_format_u64_digits(magnitude, buf, &len);
    return rte_string_append_bytes(dest, buf, len);
}
...
rte_status_t rte_string_append_u32(rte_string_t *dest, uint32_t value)
{
    ...
    st = rte_cast_u32_to_u64(value, &widened);
    if (st != RTE_STATUS_OK) /* GCOVR_EXCL_START - only fails for a NULL out
                                * pointer; &widened is always valid. */
    {
        return st;
    } /* GCOVR_EXCL_STOP */
    return rte_string_append_u64(dest, widened);
}
...
rte_status_t rte_string_append_i32(rte_string_t *dest, int32_t value)
{
    ...
    return rte_string_append_i64(dest, widened);
}
...
rte_status_t rte_string_append_hex_u32(rte_string_t *dest, uint32_t value, uint8_t min_digits)
{
    ...
    return rte_string_append_bytes(dest, out, n);
}
...
rte_status_t rte_string_to_i64(const rte_string_t *str, int64_t *out_value)
{
    ...
    {
        return RTE_STATUS_INVALID_PARAM; /* "-" with no digits */
    }
...
rte_status_t rte_string_to_u32(const rte_string_t *str, uint32_t *out_value)
{
    ...
    st = rte_string_to_u64(str, &v);
    if (st != RTE_STATUS_OK)
    {
        return st;
    }
    return rte_cast_u64_to_u32(v, out_value);
}
...
rte_status_t rte_string_to_i32(const rte_string_t *str, int32_t *out_value)
{
    ...
    st = rte_string_to_i64(str, &v);
    if (st != RTE_STATUS_OK)
    {
        return st;
    }
    return rte_cast_i64_to_i32(v, out_value);
}
