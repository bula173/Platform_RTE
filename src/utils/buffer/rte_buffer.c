...
{
    if ((buf == NULL) || (!rte_buffer_is_valid(buf)))
    {
        return RTE_STATUS_INVALID_PARAM;
    }
    buf->length = 0U;
 ...
 *        by every rte_buffer_write_*_le/be function.
 ...
 *         RTE_STATUS_RESOURCE_EXHAUSTED if n bytes will not fit.
 */
static rte_status_t rte_buffer_append_bytes(rte_buffer_t *buf, const unsigned char *bytes, size_t n)
{
    unsigned char *dest;

    if ((buf == NULL) || (bytes == NULL) || (!rte_buffer_is_valid(buf)))
    {
        return RTE_STATUS_INVALID_PARAM;
    }
 ...
 *        Shared by every rte_buffer_read_*_le/be function.
 ...
 * @return RTE_STATUS_OK on success; RTE_STATUS_INVALID_PARAM for a NULL
 *         pointer; RTE_STATUS_RESOURCE_EXHAUSTED if offset+n exceeds
 *         buf->length.
 */
static rte_status_t rte_buffer_peek_bytes(const rte_buffer_t *buf, size_t offset,
...
rte_status_t rte_buffer_write_u16_le(rte_buffer_t *buf, uint16_t value)
{
    ...
    return rte_buffer_append_bytes(buf, bytes, sizeof(bytes));
}
...
rte_status_t rte_buffer_read_u16_le(const rte_buffer_t *buf, size_t offset, uint16_t *out_value)
{
    ...
    st = rte_buffer_peek_bytes(buf, offset, bytes, sizeof(bytes));
    if (st != RTE_STATUS_OK)
    {
        return st;
    }
