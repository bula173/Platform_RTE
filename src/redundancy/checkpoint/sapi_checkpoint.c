...
/**
 * Per-attempt receive sub-budget within one sapi_channel_checkpoint()
 ...
 * from an inbound packet (sapi_posix_backend_channel_service.c's own
 ...
 * budget and then fails, triggering SAPI_SAFESTATE_REASON_CHECKPOINT_TIMEOUT
 ...
 * SAPI_CHECKPOINT_RETRY_ROUND_MS alone only bounds a round's LENGTH from
 ...
 * SAPI_CHECKPOINT_RETRY_ROUND_MS - a long relaxed budget is unaffected)
...
/**
 * Hard cap on retry rounds within one sapi_channel_checkpoint() call -
...
sapi_status_t sapi_channel_checkpoint(sapi_voter_t *voter, const sapi_checkpoint_config_t *config)
{
    ...
    if (status == SAPI_STATUS_OK)
    {
        ...
        bool channel_confirmed[SAPI_VOTER_MAX_CHANNELS];
        uint32_t round_count = 0U;
        sapi_duration_ms_t round_cap_ms = SAPI_CHECKPOINT_RETRY_ROUND_MS;
        sapi_duration_ms_t budget_based_round_cap_ms =
            (sapi_duration_ms_t)(config->max_delay_ms / SAPI_CHECKPOINT_MIN_ROUNDS_PER_BUDGET);
         ...
         * overall max_delay_ms budget is exhausted, or SAPI_CHECKPOINT_MAX_ROUNDS
         * rounds have run - see SAPI_CHECKPOINT_RETRY_ROUND_MS's own doc for
        ...
        while ((confirmed_count < config->expected_node_count)
               && (round_count < SAPI_CHECKPOINT_MAX_ROUNDS)
               && (remaining_budget_ms(start_ms, now_ms, config->max_delay_ms) > 0U))
        {
            for (i = 0U; i < channel_count; i++)
            {
                ...
                if ((i < SAPI_VOTER_MAX_CHANNELS) && channel_confirmed[i])
                {
                    continue;
                }

                channel = sapi_voter_get_channel(voter, i);
                send_status = sapi_channel_send(channel, &arrival_msg, sizeof(arrival_msg));

                last_send_status = send_status;
                if (send_status == SAPI_STATUS_OK)
                {
                    sapi_vital_message_t reply;
                    sapi_duration_ms_t sub_budget_ms;
