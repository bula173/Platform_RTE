/** @file sapi_channel_service_flow_backend.c
 *  @brief See sapi_channel_service_flow_backend.h.
 *
 * **Lazy open, not eager** (found live, not designed up front - see the
 * RCA/OCORA Phase 4 verification history in root TODO.md): sapi_flow_open()
 * blocks synchronously for its peer's handshake, up to config->open_timeout_ms
 * (REQ-OAL-FLOW-002) - a real behavioral difference from
 * safeAPIBackendPosix's own sapi_posix_backend_channel_service, whose
 * backend_setup() fires the connect/HELLO (or binds, for LISTEN) and
 * returns immediately, deferring the actual peer handshake to the first
 * backend_read() call, retried indefinitely and silently in the background
 * from then on. That difference matters for real: an integrator (e.g.
 * safeAPIRBC2oo2GP) may legitimately set up a channel whose peer is not
 * running yet, or never runs in a given deployment (e.g. an optional
 * service) - the POSIX backend tolerates that gracefully (setup() never
 * blocks on it), this backend's naive first version did not (setup() would
 * block for the full open_timeout_ms, then FAIL setup() outright, taking
 * down the whole integrator process even though the missing peer was
 * expected and harmless). Fixed by matching the POSIX backend's own
 * lazy-connect shape: backend_setup() only resolves and stores
 * configuration; the actual sapi_flow_open() call happens lazily, on the
 * first backend_read()/backend_send(), using a bounded 0ms probe timeout so
 * a still-absent peer returns SAPI_STATUS_TIMEOUT immediately rather than
 * blocking that cycle's I/O - the caller's own retry loop (e.g.
 * safeAPIRBC2oo2GP's channel_service_setup_with_retry(), or simply calling
 * read()/send() again next cycle) is what re-attempts the open, exactly the
 * same shape the POSIX backend's own backend_read() reconnect-on-demand
 * logic already has.
 */
#include "safeapi/redundancy/channel_service/sapi_channel_service_flow_backend.h"

#include <string.h>

#include "safeapi/oal/flow/sapi_flow.h"
#include "safeapi/oal/memory/sapi_mem_util.h"

/* Only the fields ensure_open() needs to rebuild a sapi_flow_config_t are
 * stored (not the whole struct) - this backend's storage must fit inside
 * sapi_channel_service_storage_t's 128-byte reserved size alongside
 * sapi_flow_storage_t (64 bytes) itself. */
typedef struct
{
    sapi_flow_storage_t   flow_storage;
    sapi_flow_handle_t    flow_handle;
    uint32_t              oflags;
    size_t                message_size;
    char                  name_buf[SAPI_CHANNEL_SERVICE_NAME_SIZE];
    bool                  resolved;   /**< setup() succeeded; the fields above are valid. */
    bool                  opened;     /**< flow_handle is valid (lazy-opened). */
} flow_channel_state_t;

typedef char flow_channel_state_fits_[(sizeof(flow_channel_state_t) <=
                                        sizeof(sapi_channel_service_storage_t)) ? 1 : -1];

static sapi_channel_service_flow_resolve_fn s_resolver;
static void *s_resolver_context;

static flow_channel_state_t *channel_state(sapi_channel_service_storage_t *storage)
{
    return (flow_channel_state_t *)(void *)storage;
}

/** CONNECT->PUBLISHER, LISTEN->SUBSCRIBER, matching safeAPIFlowBackendDDS's
 *  own oflags->netlink-role mapping in reverse (see that project's file
 *  header comment) - kept here, not there, since this module must not
 *  depend on any concrete sapi_flow_backend_t implementation. */
static uint32_t oflags_for_role(sapi_netlink_role_t role)
{
    return (role == SAPI_NETLINK_ROLE_CONNECT) ? (uint32_t)SAPI_FLOW_O_PUBLISHER
                                                : (uint32_t)SAPI_FLOW_O_SUBSCRIBER;
}

static sapi_status_t backend_setup(sapi_channel_service_storage_t *storage, const char *channel_name)
{
    flow_channel_state_t *state;
    sapi_netlink_config_t resolved;
    sapi_status_t status;

    if ((storage == NULL) || (channel_name == NULL) || (s_resolver == NULL))
    {
        return SAPI_STATUS_INVALID_PARAM;
    }
    state = channel_state(storage);
    sapi_mem_set(state, 0, sizeof(*state));

    sapi_mem_set(&resolved, 0, sizeof(resolved));
    status = s_resolver(channel_name, &resolved, s_resolver_context);
    if (status != SAPI_STATUS_OK)
    {
        return status;
    }

    (void)strncpy(state->name_buf, channel_name, sizeof(state->name_buf) - 1U);
    state->oflags = oflags_for_role(resolved.role);
    state->message_size = resolved.message_size;
    state->resolved = true;
    state->opened = false;
    return SAPI_STATUS_OK;
}

/** Attempts the deferred sapi_flow_open() exactly once, bounded by
 *  open_timeout_ms - the SAME timeout_ms the caller passed to THIS
 *  read()/send() call, matching safeAPIBackendPosix's own backend_read()
 *  reconnect-on-demand logic exactly (it hands the caller's own read
 *  timeout to its ACK-consumption poll(), not a separate hardcoded value -
 *  see that file's backend_read()). A caller sees SAPI_STATUS_TIMEOUT (or
 *  another sapi_flow_open() failure) as an ordinary per-call read()/send()
 *  failure, not a setup()-time one. Using the caller's own timeout (not a
 *  hardcoded 0) matters for real: a genuinely-present peer (e.g. a
 *  loopback peer that is also just starting up) needs a real, non-zero
 *  window to complete its handshake - 0 would starve it just as
 *  thoroughly as an absent peer, found live against a real GP process
 *  (ab-peer briefly failing the same way ab-relay-util legitimately does)
 *  before this was corrected - see root TODO.md's Phase 4 entry. */
static sapi_status_t ensure_open(flow_channel_state_t *state, sapi_duration_ms_t open_timeout_ms)
{
    sapi_flow_config_t flow_cfg;
    sapi_status_t status;

    if (state->opened)
    {
        return SAPI_STATUS_OK;
    }
    sapi_mem_set(&flow_cfg, 0, sizeof(flow_cfg));
    flow_cfg.name = state->name_buf;
    flow_cfg.oflags = state->oflags;
    flow_cfg.message_size = state->message_size;
    flow_cfg.open_timeout_ms = open_timeout_ms;
    status = sapi_flow_open(&state->flow_storage, &flow_cfg, &state->flow_handle);
    if (status == SAPI_STATUS_OK)
    {
        state->opened = true;
    }
    return status;
}

static sapi_status_t backend_read(sapi_channel_service_storage_t *storage, void *data, size_t data_size,
                                   sapi_duration_ms_t timeout_ms)
{
    flow_channel_state_t *state;
    sapi_status_t status;

    if ((storage == NULL) || (data == NULL) || (data_size == 0U))
    {
        return SAPI_STATUS_INVALID_PARAM;
    }
    state = channel_state(storage);
    if (!state->resolved)
    {
        return SAPI_STATUS_NOT_INITIALIZED;
    }
    status = ensure_open(state, timeout_ms);
    if (status != SAPI_STATUS_OK)
    {
        return status;
    }
    return sapi_flow_receive(state->flow_handle, data, data_size, NULL, timeout_ms);
}

static sapi_status_t backend_send(sapi_channel_service_storage_t *storage, const void *data, size_t data_size,
                                   sapi_duration_ms_t timeout_ms)
{
    flow_channel_state_t *state;
    sapi_status_t status;

    if ((storage == NULL) || (data == NULL) || (data_size == 0U))
    {
        return SAPI_STATUS_INVALID_PARAM;
    }
    state = channel_state(storage);
    if (!state->resolved)
    {
        return SAPI_STATUS_NOT_INITIALIZED;
    }
    status = ensure_open(state, timeout_ms);
    if (status != SAPI_STATUS_OK)
    {
        return status;
    }
    return sapi_flow_send(state->flow_handle, data, data_size, SAPI_FLOW_CHANNEL_USER, timeout_ms);
}

static sapi_status_t backend_close(sapi_channel_service_storage_t *storage)
{
    flow_channel_state_t *state;
    sapi_status_t status;

    if (storage == NULL)
    {
        return SAPI_STATUS_INVALID_PARAM;
    }
    state = channel_state(storage);
    if (!state->opened)
    {
        state->resolved = false;
        return SAPI_STATUS_OK;
    }
    status = sapi_flow_close(state->flow_handle);
    state->opened = false;
    state->resolved = false;
    return status;
}

static const sapi_channel_service_backend_t g_flow_backend = {
    backend_setup,
    backend_read,
    backend_send,
    backend_close
};

sapi_status_t sapi_channel_service_flow_backend_register_resolver(sapi_channel_service_flow_resolve_fn resolver,
                                                                    void *context)
{
    if (resolver == NULL)
    {
        return SAPI_STATUS_INVALID_PARAM;
    }
    s_resolver = resolver;
    s_resolver_context = context;
    /* Mirrors safeAPIBackendPosix's sapi_posix_backend_channel_service_register_resolver()'s
     * own convention exactly - registering the resolver AND activating this
     * backend in one call - so an integrator's call site is a one-line swap
     * between the two backends (see ab_gp_channel.c). */
    return sapi_channel_service_register_backend(&g_flow_backend);
}

const sapi_channel_service_backend_t *sapi_channel_service_flow_backend(void)
{
    return &g_flow_backend;
}
