 ...
 * rte_appmanager.h for why this is a scoped, documented exception to
 * this module (and this framework)'s usual OS-agnosticism. */
/**
 * @def RTE_APPMANAGER_HAVE_POSIX_SIGNALS
 * @brief 1 when compiled on a POSIX-ish target (signal.h available) so
 *        rte_appmanager_install_default_signal_handlers() can install a
 *        real SIGINT/SIGTERM handler; 0 otherwise, in which case that
 *        function returns RTE_STATUS_NOT_SUPPORTED.
 */
#if defined(__unix__) || defined(__APPLE__) || defined(__linux__)
#define RTE_APPMANAGER_HAVE_POSIX_SIGNALS 1
#include <signal.h>
#include <string.h>
#else
#define RTE_APPMANAGER_HAVE_POSIX_SIGNALS 0
#endif

/** @brief Global application state (single instance; no dynamic allocation). */
static rte_appmanager_state_t g_app_state = {
    .state = RTE_APP_STATE_UNINITIALIZED,
    .iteration_count = 0,
    .error_count = 0,
    .last_error = RTE_STATUS_OK
};

/** @brief Set by the installed signal handler (or rte_appmanager_request_shutdown())
 *         to request that rte_appmanager_run()'s loop exit cleanly. */
 ...
 *                        from rte_appmanager_config_t::error_threshold.
 * @return true if status is RTE_STATUS_OK (caller should proceed to the
 ...
 *         been updated).
 */
static bool rte_appmanager_handle_stage_result(rte_status_t status,
                                                  const char *stage_name,
                                                  uint32_t error_threshold)
{
    bool ok = (status == RTE_STATUS_OK);
    ...
    if (!ok) {
        ...
        rte_log_write(RTE_LOG_LEVEL_ERROR, "APPMANAGER", msg);
 ...
 *         rte_appmanager_checkpoint_mark()'s own doc in the header. Reset
 *         once per cycle by rte_appmanager_checkpoint_signature_reset();
 *         folded by every rte_appmanager_checkpoint_mark() call in
 *         between. Deliberately NOT part of the public rte_appmanager_state_t
 *         (no consumer needs to read it directly - only the folded
 *         checkpoint_id, computed on demand by
 *         rte_appmanager_checkpoint_fold_signature(), leaves this file). */
static uint64_t g_checkpoint_signature;

/** @brief True once g_checkpoint_signature has been reset for the CURRENT
 *         rte_appmanager_run() cycle - see rte_appmanager_checkpoint_mark()'s
 ...
 *        for the current cycle - called once per rte_appmanager_run()
 *        loop iteration, before pre_execute() (ADR-034).
 */
static void rte_appmanager_checkpoint_signature_reset(void)
{
    g_checkpoint_signature = RTE_APPMANAGER_CHECKPOINT_SIGNATURE_SEED;
    g_checkpoint_signature_active = true;
}

/**
 * @brief Packs a uint64_t into an 8-byte buffer, explicit little-endian -
 *        same convention rte_checkpoint.c's own build_arrival_message()
 ...
 *        across a network).
 */
static void rte_appmanager_encode_u64_le(uint8_t out[8], uint64_t value)
{
    uint32_t i;
...
void rte_appmanager_checkpoint_mark(const char *file, int32_t line, const char *label)
{
    char text[128];
    uint8_t fold_buf[16];
    rte_crc64_t mark_hash;

    if (!g_checkpoint_signature_active) {
        /* Documented no-op outside an active rte_appmanager_run() cycle -
    ...
    mark_hash = rte_checksum_crc64((const uint8_t *)text, strnlen(text, sizeof(text)));

    rte_appmanager_encode_u64_le(&fold_buf[0], g_checkpoint_signature);
    rte_appmanager_encode_u64_le(&fold_buf[8], (uint64_t)mark_hash);

    g_checkpoint_signature = (uint64_t)rte_checksum_crc64(fold_buf, sizeof(fold_buf));
}
...
int rte_appmanager_run(const rte_appmanager_config_t *config)
{
    ...
    if (config == NULL || config->ops == NULL) {
        rte_log_write(RTE_LOG_LEVEL_ERROR, "APPMANAGER", "Invalid rte_appmanager_config_t");
        return EXIT_FAILURE;
    }
    ...
    if (config->ops->init == NULL ||
        ...
        rte_log_write(RTE_LOG_LEVEL_ERROR, "APPMANAGER", "Incomplete rte_appmanager_operations_t");
        return EXIT_FAILURE;
    }
     ...
     * integrator) calling rte_appmanager_run() again only AFTER a
     * previous call has fully returned (state SHUTDOWN/ERROR by then) is
     * unaffected - see rte_lifecycle.h's own doc on why this remains a
     * single-process-wide-instance model, not multi-instance. */
    if ((g_app_state.state == RTE_APP_STATE_INITIALIZING) ||
        (g_app_state.state == RTE_APP_STATE_RUNNING) ||
        (g_app_state.state == RTE_APP_STATE_SHUTTING_DOWN)) {
        char msg[256];

        (void)snprintf(msg, sizeof(msg),
                        "rte_appmanager_run() called while an application is already running "
     ...
     * before rte_appmanager_run() is even called) - the per-cycle call
     * below already handles a NULL voter safely (rte_channel_checkpoint()
     * returns RTE_STATUS_INVALID_PARAM, handled identically to any other
    ...
    g_app_state.state = RTE_APP_STATE_INITIALIZING;
    g_app_state.iteration_count = 0;
    g_app_state.error_count = 0;
    g_app_state.last_error = RTE_STATUS_OK;
    g_shutdown_requested = 0;
    /* ADR-034: no cycle is active yet - any RTE_CHECKPOINT_MARK() called
     * from init() is a documented no-op (see rte_appmanager_checkpoint_mark()'s
     ...
     * phase - see rte_lifecycle.h's own doc on why this reset lives here
    ...
    /* Startup banner */
    {
        ...
        rte_log_write(RTE_LOG_LEVEL_INFO, "APPMANAGER", msg);
    }
    ...
    if (status != RTE_STATUS_OK) {
        ...
        rte_log_write(RTE_LOG_LEVEL_ERROR, "APPMANAGER", msg);
        g_app_state.state = RTE_APP_STATE_ERROR;
        g_app_state.last_error = status;
        g_app_state.error_count++;
     ...
     * _register_channel(), rte_cross_comparator_init()/
     * _register_channel(), rte_watchdog_create()) now rejects with
     * RTE_STATUS_INVALID_STATE for the rest of this run - see
     * rte_lifecycle.h's own doc for the deliberately-excluded
    ...
    while (!g_shutdown_requested &&
         ...
         * of this cycle's own work runs, so RTE_CHECKPOINT_MARK() calls
         * made during pre_execute()/execute()/post_execute() below fold
         * into a signature that represents ONLY this cycle's own path -
         * see rte_appmanager_checkpoint_mark()'s own doc. */
        rte_appmanager_checkpoint_signature_reset();
        ...
        if (config->ops->pre_execute != NULL) {
            ...
            if (!rte_appmanager_handle_stage_result(status, "pre_execute", config->error_threshold)) {
                continue;
            }
        }
        ...
        if (!rte_appmanager_handle_stage_result(status, "execute", config->error_threshold)) {
            continue;
        }
        ...
        if (config->ops->post_execute != NULL) {
            ...
            (void)rte_appmanager_handle_stage_result(status, "post_execute", config->error_threshold);
        }
         ...
         * into this cycle's signature (RTE_CHECKPOINT_MARK()) - using
         * that folded signature as the checkpoint_id. See
         * rte_appmanager_checkpoint_config_t's own doc for why this
         ...
         * known down (see rte_appmanager_config_t's own doc).
         *
         ...
         * checkpointing happened to be active this cycle). */
        {
            ...
            if ((config->checkpoint != NULL) && (config->checkpoint->voter != NULL)) {
                rte_checkpoint_config_t checkpoint_cfg;

                checkpoint_cfg.checkpoint_id = rte_appmanager_checkpoint_fold_signature(g_checkpoint_signature);
                ...
                status = rte_channel_checkpoint(config->checkpoint->voter, &checkpoint_cfg);
                committed = rte_appmanager_handle_stage_result(status, "checkpoint", config->error_threshold);
            ...
            if (config->ops->on_checkpoint_result != NULL) {
                ...
                (void)rte_appmanager_handle_stage_result(status, "on_checkpoint_result", config->error_threshold);
            }
        }
    }
    ...
    g_app_state.state = RTE_APP_STATE_SHUTTING_DOWN;
    /* ADR-034: the cycle loop has exited - any RTE_CHECKPOINT_MARK() called
     ...
     * legitimately runs BETWEEN two rte_appmanager_run() calls (e.g. a
     * test fixture - or integrator code - that builds the NEXT run's own
     * voter/channels/timers before calling rte_appmanager_run() again)
     * would be incorrectly rejected: it runs after this run's own
     * rte_lifecycle_lock() but before the next run's own unlock. */
    rte_lifecycle_unlock();
    ...
    if (status != RTE_STATUS_OK) {
        ...
        rte_log_write(RTE_LOG_LEVEL_WARNING, "APPMANAGER", msg);
        /* Continue anyway; shutdown must complete */
    }

    g_app_state.state = RTE_APP_STATE_SHUTDOWN;

    /* Final summary - the iteration count is this event's natural Cycle
     * field, so this uses rte_log_write_event() (structured), not
     * rte_log_write() like this function's other, cycle-less messages
     * above. */
    {
        char extra[64];
        ...
        rte_log_write_event(RTE_LOG_LEVEL_INFO, "", g_app_state.iteration_count, "APPMANAGER", "-", "SUMMARY",
...
{
    g_app_state.state = RTE_APP_STATE_UNINITIALIZED;
    g_app_state.iteration_count = 0;
    g_app_state.error_count = 0;
    g_app_state.last_error = RTE_STATUS_OK;
    g_shutdown_requested = 0;
...
#if RTE_APPMANAGER_HAVE_POSIX_SIGNALS

/* Async-signal-safe: writes one volatile int, nothing else (no I/O, no
 * allocation) - see rte_appmanager_request_shutdown()'s own definition
 * above. */
static void rte_appmanager_signal_handler(int signum)
{
    (void)signum;
...
rte_status_t rte_appmanager_install_default_signal_handlers(void)
{
    ...
    sa.sa_handler = rte_appmanager_signal_handler;
    if (sigemptyset(&sa.sa_mask) != 0)
    {
        return RTE_STATUS_INTERNAL_ERROR; /* GCOVR_EXCL_LINE */
    }
    sa.sa_flags = 0;
...
#endif /* RTE_APPMANAGER_HAVE_POSIX_SIGNALS */
