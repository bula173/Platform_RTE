 ...
 * same trigger ADR-025 documents for `sapi_cross_comparator`.
 *
 * This header declares the STORAGE SHAPE only
 * (SAFEAPI_DECLARE_CALLBACK_LIST below) - it does not generate
 * init/register/dispatch functions. That is a deliberate choice, not an
 * omission: `sapi_voter`/`sapi_cross_comparator`'s own registered
 * callbacks (`sapi_voter_compare_fn`, `on_disagreement`, ...) are each a
 ...
 * types (SAFEAPI_DECLARE_STORAGE, sapi_types.h, is type-only too). A
 ...
 *   SAFEAPI_DECLARE_CALLBACK_LIST(my_validator_list_t, my_validator_fn, 2U);
 *
 * - then write your own small, ordinary (non-macro) init()/register()/
 * dispatch() functions against `my_validator_list_t`, following this
 * shape (every setup-only constructor in this framework - `sapi_timer_create()`,
 * `sapi_voter_init()`/`_register_channel()`, `sapi_cross_comparator_init()`/
 * `_register_channel()`, `sapi_watchdog_create()` - calls
 * `sapi_lifecycle_check_setup_allowed()`, safeapi/lifecycle/sapi_lifecycle.h,
 ...
 *
 *   static sapi_status_t my_validator_list_init(my_validator_list_t *list)
 *   {
 *       uint32_t i;
 *       if (list == NULL) { return SAPI_STATUS_INVALID_PARAM; }
 *       list->count = 0U;
 *       for (i = 0U; i < 2U; i++) { list->slots[i].fn = NULL; list->slots[i].context = NULL; }
 *       return SAPI_STATUS_OK;
 *   }
 *
 *   static sapi_status_t my_validator_list_register(my_validator_list_t *list, my_validator_fn fn, void *context)
 *   {
 *       sapi_status_t lifecycle_status;
 *       if ((list == NULL) || (fn == NULL)) { return SAPI_STATUS_INVALID_PARAM; }
 *       lifecycle_status = sapi_lifecycle_check_setup_allowed();
 *       if (lifecycle_status != SAPI_STATUS_OK) { return lifecycle_status; }
 *       if (list->count >= 2U) { return SAPI_STATUS_RESOURCE_EXHAUSTED; }
 ...
 *   // custom-callback precedent as sapi_watchdog_create()).
 ...
 * @{
 */
#ifndef SAFEAPI_COMMON_NOTIFY_H
#define SAFEAPI_COMMON_NOTIFY_H
...
/**
 * @def SAFEAPI_DECLARE_CALLBACK_LIST
