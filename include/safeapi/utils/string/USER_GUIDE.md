 ...
 * rte_status_t rc = rte_strncpy(dest, src, 32);
 * if (rc != RTE_STATUS_OK) {
 *     log_error("String copy failed: %s", rte_status_to_string(rc));
 * }
 * ```
 *
 ...
 * strcpy(path, "/home/");
 *
 * rte_status_t rc = rte_strncat(path, username, 256);
 ...
 *
 * ```c
 * size_t len = rte_strnlen(str, 256);
 * if (len == 256) {
 ...
 * ```c
 * // Copy: Bounded strcpy
 * rte_status_t rte_strncpy(char *dest, const char *src, size_t max_len);
 *
 * // Concatenate: Bounded strcat
 * rte_status_t rte_strncat(char *dest, const char *src, size_t max_len);
 *
 * // Length: With maximum limit
 * size_t rte_strnlen(const char *str, size_t max_len);
 *
 * // Comparison: Bounded strcmp
 * int rte_strncmp(const char *a, const char *b, size_t max_len);
 *
 * // Format: Bounded sprintf
 * rte_status_t rte_snprintf(char *buf, size_t max_len, const char *fmt, ...);
 * ```
 *
 ...
 *
 *     // Process command
 *     if (rte_strncmp(cmd, "status", 32) == 0) {
 *         handle_status(arg);
 *     } else if (rte_strncmp(cmd, "reset", 32) == 0) {
 *         handle_reset(arg);
 *     }
 * }
 * ```
 ...
 *     char buffer[256];
 *     rte_status_t rc = rte_snprintf(buffer, sizeof(buffer),
 *         "[%u] EVENT: %s (code=%d) - %s",
 ...
 *    - RTE_STATUS_OK: Success, string complete
 *    - RTE_STATUS_RESOURCE_EXHAUSTED: String truncated/overflow
 *    - RTE_STATUS_INVALID_PARAM: Bad input
 *
