 ...
 *
 * A sibling to sapi_ipc (ADR-001 section 4, service 5), not a
 * replacement: sapi_ipc models a bounded local queue between two ends
 * that already exist (e.g. a pipe created by one process for two of its
 * own threads/tasks). sapi_netlink models a connection-oriented link
 ...
 * onto sapi_ipc_config_t (ADR-005's backend-agnostic config principle:
 ...
 * REQ-OAL-NETLINK-002: sapi_netlink_open() shall never block longer than
 ...
 * decides on its own.
 */
typedef enum sapi_netlink_role_e
{
    SAPI_NETLINK_ROLE_LISTEN  = 0, /**< Bind config->port and accept one peer. */
...
typedef struct sapi_netlink_config_s
{
    ...
    size_t               message_size;
    /** Maximum time sapi_netlink_open() may block establishing the
     *  link (REQ-OAL-NETLINK-002). */
    sapi_duration_ms_t   connect_timeout_ms;
} sapi_netlink_config_t;
 ...
 *         SAPI_STATUS_TIMEOUT if the link is not established within
 *         config->connect_timeout_ms; SAPI_STATUS_NOT_INITIALIZED if no
 *         backend is registered (sapi_netlink_register_backend());
 *         SAPI_STATUS_NOT_SUPPORTED if the registered backend does not
 *         implement open.
 * REQ-OAL-NETLINK-010
 */
