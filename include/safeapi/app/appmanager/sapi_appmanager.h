 ...
 * Per ADR-019/ADR-034, it can also be configured (sapi_appmanager_config_t::checkpoint)
 * to perform a bounded sapi_channel_checkpoint() rendezvous (ADR-017) as the
 ...
 * see sapi_appmanager_checkpoint_config_t's own doc). Instead, application
 * code calls SAPI_CHECKPOINT_MARK() (or the labelled variant,
 * SAPI_CHECKPOINT_MARK_LABEL()) at meaningful decision/preparation points
...
typedef struct {
     ...
     * sapi_appmanager_run()'s doc for the stage order). Intended for a
     ...
     * sapi_channel_checkpoint() itself returned non-OK (a genuine
     * validation failure or timeout - REQ-CHECKPOINT-003 has already
     * driven this process to SAPI_SAFESTATE_LEVEL_SAFE by the time this
     ...
     * @return SAPI_STATUS_OK on normal handling; other codes for error
 ...
 * calls sapi_channel_checkpoint() once at the END of every cycle - after
 ...
 * running sapi_appmanager_state_t::iteration_count as checkpoint_id, on the
 ...
 * history.
 *
 * Leave sapi_appmanager_config_t::checkpoint NULL to disable this entirely
 * (the default); this is opt-in because sapi_channel_checkpoint() blocks
 ...
 *        sapi_appmanager_checkpoint_fold_signature()'s own doc. A cycle in
 *        which no SAPI_CHECKPOINT_MARK() call is ever made therefore
 *        always computes checkpoint_id as
 *        `sapi_appmanager_checkpoint_fold_signature(SAPI_APPMANAGER_CHECKPOINT_SIGNATURE_SEED)`
 ...
 *        uint32_t sapi_checkpoint_config_t::checkpoint_id expects (ADR-034).
 *
 ...
 *        signature (ADR-034).
 *
 * Intended to be called via SAPI_CHECKPOINT_MARK() / SAPI_CHECKPOINT_MARK_LABEL()
 ...
 * same explicit-little-endian-byte-pack convention sapi_checkpoint.c's own
 ...
 * are NULL, and outside a sapi_appmanager_run() cycle (before the loop
...
/**
 * @def SAPI_CHECKPOINT_MARK
 * @brief Records a checkpoint mark for this call site (ADR-034) - see
 *        sapi_appmanager_checkpoint_mark()'s own doc.
 */
#define SAPI_CHECKPOINT_MARK() sapi_appmanager_checkpoint_mark(__FILE__, (int32_t)__LINE__, NULL)

/**
 * @def SAPI_CHECKPOINT_MARK_LABEL
 * @brief Records a checkpoint mark identified by a caller-supplied label
 *        instead of this call site's file:line (ADR-034) - see
 *        sapi_appmanager_checkpoint_mark()'s own doc.
 */
#define SAPI_CHECKPOINT_MARK_LABEL(label_str) sapi_appmanager_checkpoint_mark(__FILE__, (int32_t)__LINE__, (label_str))
 ...
 *      see sapi_appmanager_checkpoint_config_t's own doc for why):
