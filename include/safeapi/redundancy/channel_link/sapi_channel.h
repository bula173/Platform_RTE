 ...
 *
 * REDESIGNED (ADR-025): prior to this, `sapi_channel_t` wrapped N
 * redundant channels *and* did 2oo2/2oo3/NMR voting between them in one
 * type. That voting engine has moved to `sapi_voter` (register N
 * `sapi_channel_t` instances into a `sapi_voter_t`); a new
 * `sapi_cross_comparator` does the same for exactly 2 channels. This
 ...
 * dispatch to a backend callback" shape as `sapi_timer`/`sapi_nvm`
...
#ifndef SAPI_CHANNEL_LINK_H
#define SAPI_CHANNEL_LINK_H
...
typedef struct {
     ...
     *  owner - a sapi_voter_t/sapi_cross_comparator_t - after enough
 ...
 * @return SAPI_STATUS_OK on success; SAPI_STATUS_TIMEOUT if the
 *         operation times out; SAPI_STATUS_HARDWARE_FAULT on any other
 *         send error.
 */
typedef sapi_status_t (*sapi_channel_send_fn)(void *channel_handle,
                                               const void *data,
                                               size_t data_size);
 ...
 *         receive error.
 */
typedef sapi_status_t (*sapi_channel_recv_fn)(void *channel_handle,
                                               void *data,
                                               size_t data_size,
                                               uint32_t timeout_ms);

/**
 * @brief Configuration for sapi_channel_init().
 */
typedef struct {
     ...
     *  (sapi_voter_get_channel_by_name()) and logging - same
     *  caller-owned-pointer convention as sapi_watchdog_config_t::name:
     *  not copied, so it must outlive this channel (a string literal is
     *  the common case). May be NULL, in which case this channel is
     *  never matched by sapi_voter_get_channel_by_name(). */
    const char *name;
 ...
 *
 * Caller allocates this structure and passes it to sapi_channel_init().
 * No dynamic memory.
 */
typedef struct {
    sapi_channel_config_t config;  /**< Configuration this instance was initialized with. */
    sapi_channel_health_t health;  /**< This channel's own health/statistics. */
    bool initialized;                    /**< Set true by sapi_channel_init(). */
} sapi_channel_storage_t;

/**
 * @brief Opaque handle to a vital channel instance.
 *
 * Created by sapi_channel_init(); used directly, or registered
 * into a sapi_voter_t (N-way voting) or sapi_cross_comparator_t
 ...
 * @return SAPI_STATUS_INVALID_PARAM if storage or config is NULL, or
 ...
 *
 * @return SAPI_STATUS_OK on success
 * @return SAPI_STATUS_INVALID_PARAM if handle/data is NULL, data_size
 *         is 0, or handle was never initialized
 * @return Whatever config->send() itself returns on failure (typically
 *         SAPI_STATUS_TIMEOUT or SAPI_STATUS_HARDWARE_FAULT)
 *
 ...
 *
 * @return SAPI_STATUS_OK on success
 * @return SAPI_STATUS_INVALID_PARAM if handle or out_health is NULL
 *
 ...
 *        sapi_channel_config_t::name at sapi_channel_init() time.
 *
 * @param[in] handle  Vital channel handle. Must not be NULL.
 ...
 *
 * Intended for a sapi_voter_t/sapi_cross_comparator_t (or any other
 ...
 *
 * @return SAPI_STATUS_OK always (including when handle is NULL).
 *
