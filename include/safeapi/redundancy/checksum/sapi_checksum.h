 ...
 */
typedef struct {
    sapi_crc64_t computed;  /**< CRC-64 computed from data */
    sapi_crc64_t expected;  /**< CRC-64 from message header/trailer */
    uint8_t match;          /**< 1 if match, 0 if mismatch */
} sapi_checksum_result_t;
 ...
 * @code
 * // At startup
 * sapi_checksum_crc64_init(SAPI_CRC64_ERTMS);
 *
...
typedef struct {
    ...
    sapi_crc64_t crc64;        /**< CRC-64 of entire message including payload */
 ...
 *
 * sapi_status_t status = sapi_checksum_vital_message_verify(
 *     &received_msg,
