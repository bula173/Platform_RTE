 ...
 * is sapi_redundancy_config_t::standby_mode's job; carrying the encoded
 ...
 * @{
 */
#ifndef SAPI_STATE_TRANSFER_H
#define SAPI_STATE_TRANSFER_H
...
#define SAPI_STATE_TRANSFER_MAX_FIELDS 8U

/**
 * @brief Optional custom wire encoder for one field, registered via
 *        sapi_state_transfer_register_field_ex() - e.g. a field whose live
 ...
 *        contract sapi_state_transfer_encode()'s own offset bookkeeping
 ...
 * @return SAPI_STATUS_OK on success; any other value aborts the whole
 *         sapi_state_transfer_encode() call with that status.
 */
typedef sapi_status_t (*sapi_state_field_encode_fn)(const void *data, uint8_t *out, size_t size);

/**
 * @brief Inverse of sapi_state_field_encode_fn - must consume EXACTLY
 *        `size` bytes from in and write the result into *data.
 * @return SAPI_STATUS_OK on success; any other value aborts the whole
 *         sapi_state_transfer_decode() call with that status.
 */
typedef sapi_status_t (*sapi_state_field_decode_fn)(void *data, const uint8_t *in, size_t size);
...
typedef struct {
    ...
    size_t      size;
    /** NULL for a plain (raw memcpy) field. See sapi_state_field_encode_fn. */
    sapi_state_field_encode_fn encode_fn;
    /** NULL for a plain (raw memcpy) field. See sapi_state_field_decode_fn.
     *  Must be NULL iff encode_fn is NULL (both-or-neither, checked by
     *  sapi_state_transfer_register_field_ex()). */
    sapi_state_field_decode_fn decode_fn;
} sapi_state_field_t;

/** Caller-owned storage for one registry instance. */
typedef struct {
    sapi_state_field_t fields[SAPI_STATE_TRANSFER_MAX_FIELDS];
    uint32_t            field_count;
} sapi_state_transfer_registry_t;

/**
 * @brief Initializes an empty registry.
 * @param[out] registry  Must not be NULL.
 * @return SAPI_STATUS_OK; SAPI_STATUS_INVALID_PARAM if registry is NULL.
 */
 ...
 * @return SAPI_STATUS_OK; SAPI_STATUS_INVALID_PARAM (NULL registry/data, or
 *         size 0); SAPI_STATUS_RESOURCE_EXHAUSTED if the registry already
 *         holds SAPI_STATE_TRANSFER_MAX_FIELDS entries.
 */
 ...
 *        sapi_state_field_t's own doc for when this is needed (e.g. a
 ...
 * @return SAPI_STATUS_OK; SAPI_STATUS_INVALID_PARAM (any required pointer
 *         NULL, or size 0); SAPI_STATUS_RESOURCE_EXHAUSTED as in
 *         sapi_state_transfer_register_field().
 */
 ...
 *                       sapi_state_transfer_encoded_size(registry).
 * @param[out] out_len   Receives the number of bytes actually written
 *                       (== sapi_state_transfer_encoded_size(registry) on
 *                       success). May be NULL.
 * @return SAPI_STATUS_OK; SAPI_STATUS_INVALID_PARAM (NULL registry/out);
 *         SAPI_STATUS_RESOURCE_EXHAUSTED if out_size is too small.
 */
 ...
 *                      sapi_state_transfer_encoded_size(registry).
 * @return SAPI_STATUS_OK; SAPI_STATUS_INVALID_PARAM (NULL registry/in);
 *         SAPI_STATUS_RESOURCE_EXHAUSTED if in_len is too small.
