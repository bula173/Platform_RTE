/**
 * @file rte_checkpoint.h
 * @brief Bounded checkpoint rendezvous for distributed vital channels (ADR-017)
 *
 * Implements rte_channel_checkpoint(), already specified (but never built)
 ...
 * message is a rte_vital_message_t (rte_checksum.h), verified with
 * rte_checksum_vital_message_verify() before it is allowed to count
 * toward quorum.
 *
 ...
 * @{
 *
 * REQ-CHECKPOINT-001: rte_channel_checkpoint() shall never block longer
 ...
 *                     arrive within max_delay_ms, rte_channel_checkpoint()
 *                     shall call rte_safestate_enter() at
 *                     RTE_SAFESTATE_LEVEL_SAFE with
 *                     RTE_SAFESTATE_REASON_CHECKPOINT_TIMEOUT before
 *                     returning RTE_STATUS_TIMEOUT.
 */

#ifndef RTE_CHECKPOINT_H
#define RTE_CHECKPOINT_H
...
typedef struct
{
     ...
     *  rte_vital_message_t, so a stale or wrong-cycle reply is rejected
     ...
     *  deployment's known network/clock jitter - see rte_clocksync.h -
     ...
     *  internal retry round (see rte_channel_checkpoint()'s own doc) as
    ...
    rte_watchdog_t watchdog;
} rte_checkpoint_config_t;
 ...
 * rte_channel_send()), then polls each channel's
 * rte_channel_receive() bounded by config->max_delay_ms, counting
 * only replies that pass rte_checksum_vital_message_verify() and carry
 * the matching checkpoint_id. This is direct per-channel I/O, not a
 * rte_voter_send()/_receive() voting round - checkpoint messages are
 * rendezvous markers, not data to vote on.
 *
 * @param voter   Voter with 1+ channels already registered
 *                (rte_voter_register_channel()). Must not be NULL.
 * @param config  Checkpoint configuration. Must not be NULL.
 * @return RTE_STATUS_OK if at least config->expected_node_count valid
 ...
 * @return RTE_STATUS_INVALID_PARAM if voter or config is NULL, or
 *         config->expected_node_count exceeds voter's registered channel count.
 * @return RTE_STATUS_TIMEOUT if fewer than expected_node_count valid
 *         confirmations arrived in time.
 *
 * @pre voter != NULL, config != NULL
 * @post On RTE_STATUS_TIMEOUT, rte_safestate_enter() has already been
 *       called at RTE_SAFESTATE_LEVEL_SAFE with
 *       RTE_SAFESTATE_REASON_CHECKPOINT_TIMEOUT before this function
 *       returns (REQ-CHECKPOINT-003) - the same "safe-state is triggered
 *       automatically by the sync/vote logic itself" pattern
 *       rte_voter_receive() already uses on a voting disagreement.
 *
 * @safety No dynamic memory allocation. Never blocks longer than
 *         config->max_delay_ms (REQ-CHECKPOINT-001).
 */
rte_status_t rte_channel_checkpoint(rte_voter_t *voter,
