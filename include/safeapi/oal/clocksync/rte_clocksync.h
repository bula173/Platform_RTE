 ...
 * `rte_<service>_backend_t` + `rte_<service>_register_backend()` pattern
 ...
 *                    RTE_STATUS_NOT_INITIALIZED if no backend has been
 *                    registered.
...
/*
 * The backend vtable (rte_clocksync_backend_t) and
 * rte_clocksync_register_backend() live in
 * safeapi_backend/clocksync/rte_clocksync_backend.h, not here (ADR-021).
 ...
 *         NULL; RTE_STATUS_NOT_INITIALIZED if no backend is registered
