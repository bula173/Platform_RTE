 ...
 *
 * REQ-OAL-PLATFORM-001: sapi_platform_realtime_init() is best-effort - a
 ...
 *                       still return SAPI_STATUS_OK, having applied what it
 ...
 *        sapi_timer_* / sapi_task_* thread is created.
 *
 ...
 *                     range.
 * @return SAPI_STATUS_INVALID_PARAM if rt_priority > 99.
 *         SAPI_STATUS_NOT_INITIALIZED if no backend is registered.
 *         SAPI_STATUS_NOT_SUPPORTED if a backend is registered but does
 *         not implement this operation.
 *         SAPI_STATUS_OK otherwise - including when the backend could only
