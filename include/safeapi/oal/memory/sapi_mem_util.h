 ...
 *        libc's <string.h> directly for - sapi_mem_set()/sapi_mem_copy()/
 *        sapi_mem_compare() are the ONE sanctioned call site for each
 ...
 * OAL services - sapi_timer, sapi_nvm, sapi_memory, etc. - see
 * sapi_appmanager.h's own note on that distinction) - same convention as
 * sapi_notify.h's SAFEAPI_DECLARE_CALLBACK_LIST (ADR-030).
 */
...
static inline void sapi_mem_set(void *dest, int value, size_t count)
{
...
static inline void sapi_mem_copy(void *dest, const void *src, size_t count)
{
...
static inline int sapi_mem_compare(const void *a, const void *b, size_t count)
{
