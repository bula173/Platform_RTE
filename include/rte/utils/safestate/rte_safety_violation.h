/**
 * @file rte_safety_violation.h
 * @brief Opt-in notification hook for the three new safety primitives
 *        (safe pointer, checked integer arithmetic, bounds check): a
 *        SEPARATE, dedicated handler from rte_safestate_handler_t
 *        (rte_safestate.h) - registering it does not change any of
 ...
 *        at all.
 *
 * Why a separate mechanism from rte_safestate: unlike a checkpoint
 * timeout or a voter disagreement (which are ALWAYS genuine faults, so
 * REQ-CHECKPOINT-003/REQ-VOTER-005 escalate through rte_safestate_enter()
 * unconditionally), these three primitives are also called during
 * ordinary, expected control flow (e.g. rte_cast_bounds_check() used to
 * ask "is this the last valid index" is not a fault). Forcing every one
 * of their failures through rte_safestate_enter() would trigger a
 ...
 *       framework's own implementation (rte_safe_ptr.c/rte_cast.c),
 *       not the ultimate caller's call site - unlike RTE_ASSERT/
 *       RTE_SAFESTATE (macros, so they capture the caller's own
 *       __FILE__/__LINE__), the three primitives this module instruments
 *       are ordinary functions, not macros, per rte_cast.h's own
 ...
 * @{
 */
#ifndef RTE_COMMON_SAFETY_VIOLATION_H
#define RTE_COMMON_SAFETY_VIOLATION_H
 ...
 */
typedef enum rte_safety_violation_kind_e
{
    /** rte_safe_ptr_get()/_offset() found a corrupted canary
     *  (REQ-OAL-SAFEPTR-001) - never a normal condition. */
    RTE_SAFETY_VIOLATION_CORRUPTION = 0,
    /** A rte_cast_checked_add_*()/_sub_*()/_mul_*() call detected
     *  overflow or underflow (REQ-COMMON-CAST-004). */
    RTE_SAFETY_VIOLATION_OVERFLOW = 1,
    /** rte_cast_bounds_check() (REQ-COMMON-CAST-005) or
     *  rte_safe_ptr_offset()'s own bounds check
     *  (REQ-OAL-SAFEPTR-002) found index/offset+length outside the
     *  valid range. */
    RTE_SAFETY_VIOLATION_OUT_OF_RANGE = 2
} rte_safety_violation_kind_t;

/**
 * @brief Handler invoked by rte_safety_violation_report().
 *
 ...
 *
 * @note Unlike rte_safestate_handler_t, this handler is expected to
 *       return - it is a notification, not a fault-reaction level. Do
 *       not call rte_safestate_enter() unconditionally from inside it;
 *       that would defeat the whole point of keeping this separate from
 *       rte_safestate (see this header's own file-level doc) - make
 *       that decision deliberately, in your own handler, if that is what
 *       your application wants for a specific kind.
 */
typedef void (*rte_safety_violation_handler_t)(rte_safety_violation_kind_t kind,
                                                  const char *file,
                                                  int32_t line,
                                                  const char *message);
 ...
 *        rte_safety_violation_report(). Registering again replaces the
 ...
 *
 * Called internally by rte_safe_ptr_get()/_offset() and the
 * rte_cast_checked_*()/rte_cast_bounds_check() functions at their own
...
#endif /* RTE_COMMON_SAFETY_VIOLATION_H */
