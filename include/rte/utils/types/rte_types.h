 ...
 * @{
 */
#ifndef RTE_COMMON_TYPES_H
#define RTE_COMMON_TYPES_H
 ...
 *        (e.g. in RTE_DECLARE_STORAGE) so its address is suitable for
...
#include <stdalign.h>
#define RTE_ALIGNED_(n) _Alignas(n)
#elif defined(_MSC_VER)
#define RTE_ALIGNED_(n) __declspec(align(n))
#elif defined(__GNUC__) || defined(__clang__)
#define RTE_ALIGNED_(n) __attribute__((aligned(n)))
#else
#define RTE_ALIGNED_(n) /* no portable alignment available */
#endif

/**
 * @def RTE_DECLARE_STORAGE
 ...
 *
 * Usage: RTE_DECLARE_STORAGE(rte_timer_storage_t, 64U);
 */
#define RTE_DECLARE_STORAGE(type_name, byte_size)                   \
    typedef struct                                                      \
    {                                                                    \
        RTE_ALIGNED_(RTE_STORAGE_ALIGN)                          \
