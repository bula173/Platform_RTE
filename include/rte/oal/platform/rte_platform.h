 ...
 * entirely osadapter-defined (a PREEMPT_RT Linux OSAdapter uses
 * mlockall()/SCHED_FIFO; a bare-metal OSAdapter may do nothing at all) -
 * this header is only the osadapter-agnostic seam, so application startup
 * code never has to name a concrete OSAdapter to ask for it (ADR-005).
 *
 * REQ-OAL-PLATFORM-001: rte_platform_realtime_init() is best-effort - a
 *                       OSAdapter that cannot obtain some or all of the
 ...
 * @{
 */
#ifndef RTE_OAL_PLATFORM_H
#define RTE_OAL_PLATFORM_H
...
/**
 * @brief Applies the registered OSAdapter's real-time platform
 *        configuration for the calling process/thread. Call once at
 *        process startup, after the OSAdapter is registered and before any
 ...
 *                     clamped by the OSAdapter to the target's supported
 *                     range.
 * @return RTE_STATUS_INVALID_PARAM if rt_priority > 99.
 *         RTE_STATUS_NOT_INITIALIZED if no OSAdapter is registered.
 *         RTE_STATUS_NOT_SUPPORTED if an OSAdapter is registered but does
 *         not implement this operation.
 *         RTE_STATUS_OK otherwise - including when the OSAdapter could only
...
/*
 * The OSAdapter vtable (rte_osadapter_platform_t) and
 * rte_osadapter_platform_register() live in
 * rte_osadapter/platform/rte_osadapter_platform.h, not here (ADR-021).
