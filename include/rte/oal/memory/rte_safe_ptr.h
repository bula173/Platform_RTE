 ...
 * allocated struct), the same posture rte_checksum.c's own init-state
 * check already uses for its internal manager state. No dynamic
 * allocation: rte_safe_ptr_t wraps memory the caller already owns (a
 * static buffer, a pool block from rte_memory.h, a stack buffer whose
 ...
 *                      RTE_STATUS_DATA_CORRUPTION before any other check
 *                      runs.
 * REQ-OAL-SAFEPTR-002: rte_safe_ptr_offset() shall verify
 ...
 * REQ-OAL-SAFEPTR-003: rte_safe_ptr_invalidate() shall clear both the
 *                      wrapped pointer and the canary, so a subsequent
 *                      access on the same wrapper fails
 *                      RTE_STATUS_DATA_CORRUPTION rather than silently
 ...
 * @{
 */
#ifndef RTE_OAL_SAFE_PTR_H
#define RTE_OAL_SAFE_PTR_H
 ...
 *             fails RTE_STATUS_INVALID_PARAM, not RTE_STATUS_DATA_CORRUPTION,
 *             since the canary is still written correctly).
 * @param size Size in bytes of the region ptr points to.
 * @return RTE_STATUS_OK on success; RTE_STATUS_INVALID_PARAM if sp is
 ...
 * @return RTE_STATUS_OK on success; RTE_STATUS_INVALID_PARAM if sp or
 *         out_ptr is NULL; RTE_STATUS_DATA_CORRUPTION if sp's canary is
 ...
 * @return RTE_STATUS_OK on success; RTE_STATUS_INVALID_PARAM if sp or
 *         out_ptr is NULL; RTE_STATUS_DATA_CORRUPTION if sp's canary is
 *         wrong (REQ-OAL-SAFEPTR-001); RTE_STATUS_VALUE_OUT_OF_RANGE if
 ...
 *        every subsequent access on sp fails RTE_STATUS_DATA_CORRUPTION.
 *
 * Relevant when the underlying region is logically released back to a
 * pool (rte_mem_pool_release()) or otherwise stops being valid for this
