 ...
 * @{
 */
#ifndef RTE_OSADAPTER_SOCKET_H
#define RTE_OSADAPTER_SOCKET_H
...
#define RTE_OS_SOCKET_INVALID_HANDLE ((intptr_t)(-1))

/** @brief Socket handle representation (pointer or file descriptor). */
typedef intptr_t rte_os_socket_handle_t;

/**
 * @brief Socket transport type.
 */
typedef enum rte_os_socket_type_e
{
    RTE_OS_SOCKET_TYPE_UDP = 1, /**< Raw UDP datagram socket. */
    RTE_OS_SOCKET_TYPE_TCP = 2  /**< Raw TCP stream socket. */
} rte_os_socket_type_t;

/**
 * @brief Polling events bitmask.
 */
typedef enum rte_os_socket_event_e
{
    RTE_OS_SOCKET_EVENT_READ  = 1U, /**< Socket is ready for reading without blocking. */
    RTE_OS_SOCKET_EVENT_WRITE = 2U, /**< Socket is ready for writing without blocking. */
    RTE_OS_SOCKET_EVENT_ERROR = 4U  /**< Error condition on socket. */
} rte_os_socket_event_t;
...
typedef struct rte_os_socket_ops_s
{
     ...
     */
    rte_status_t (*bind)(rte_os_socket_handle_t handle, const char *host, uint16_t port);
     ...
     */
    rte_status_t (*connect)(rte_os_socket_handle_t handle, const char *host, uint16_t port);
     ...
     */
    rte_status_t (*send)(rte_os_socket_handle_t handle, const void *buf, size_t len, size_t *out_sent);
     ...
     * @param out_sent Optional output parameter receiving bytes actually sent.
     */
    rte_status_t (*sendto)(rte_os_socket_handle_t handle, const void *buf, size_t len,
     ...
     */
    rte_status_t (*recv)(rte_os_socket_handle_t handle, void *buf, size_t len, size_t *out_recv);
     ...
     * @param out_recv Output parameter receiving bytes actually received. Must not be NULL.
     */
    rte_status_t (*recvfrom)(rte_os_socket_handle_t handle, void *buf, size_t len,
                              char *out_host, size_t host_len, uint16_t *out_port, size_t *out_recv);

    /**
     * @brief Polls a socket for I/O readiness with timeout.
     * @param handle Valid socket handle.
     * @param events Bitmask of RTE_OS_SOCKET_EVENT_* to monitor.
     * @param timeout_ms Maximum time to wait in milliseconds.
     * @param out_revents Output bitmask of events ready. Must not be NULL.
     */
    rte_status_t (*poll)(rte_os_socket_handle_t handle, uint32_t events,
     ...
     * @param handle Valid socket handle.
     */
    rte_status_t (*close)(rte_os_socket_handle_t handle);
 ...
 * @return RTE_STATUS_OK on success, RTE_STATUS_INVALID_PARAM if ops is NULL.
 */
