...
static rte_status_t mock_open(rte_nvm_storage_t *storage,
                                ...
                                rte_nvm_handle_t *out_handle)
{
    ...
    (void)config;
    g_mock_open_calls++;
    *out_handle = (rte_nvm_handle_t)(void *)1;
    return RTE_STATUS_OK;
}

static rte_status_t mock_read(rte_nvm_handle_t handle, size_t offset,
...
static rte_status_t mock_write(rte_nvm_handle_t handle, size_t offset,
...
static rte_status_t mock_sync(rte_nvm_handle_t handle)
{
    (void)handle;
    g_mock_sync_calls++;
    return RTE_STATUS_OK;
...
static rte_status_t mock_close(rte_nvm_handle_t handle)
{
    (void)handle;
    g_mock_close_calls++;
    return RTE_STATUS_OK;
...
int main(void)
{
    rte_nvm_storage_t storage __attribute__((unused));
    rte_nvm_handle_t handle __attribute__((unused)) = NULL;

    assert(rte_nvm_open(NULL, NULL, NULL) == RTE_STATUS_INVALID_PARAM);

    rte_nvm_config_t bad_config __attribute__((unused)) = {0};
    bad_config.region_name = NULL;
    bad_config.region_size = 128U;
    assert(rte_nvm_open(&storage, &bad_config, &handle) == RTE_STATUS_INVALID_PARAM);

    /* Invalid config (region_size == 0) rejected even with a valid name. */
    rte_nvm_config_t zero_size_config __attribute__((unused)) = {0};
    zero_size_config.region_name = "train_db";
    zero_size_config.region_size = 0U;
    assert(rte_nvm_open(&storage, &zero_size_config, &handle) == RTE_STATUS_INVALID_PARAM);

    rte_nvm_config_t config __attribute__((unused)) = {0};
    ...
    assert(rte_nvm_read(handle, 0U, buf, 0U) == RTE_STATUS_INVALID_PARAM);
    assert(rte_nvm_write(NULL, 0U, buf, sizeof(buf)) == RTE_STATUS_INVALID_PARAM);
    assert(rte_nvm_write(handle, 0U, NULL, sizeof(buf)) == RTE_STATUS_INVALID_PARAM);
    assert(rte_nvm_write(handle, 0U, buf, 0U) == RTE_STATUS_INVALID_PARAM);
    ...
    assert(rte_nvm_write(dummy_handle, 0U, buf, sizeof(buf)) == RTE_STATUS_NOT_INITIALIZED);
    ...
    assert(rte_nvm_read(handle, 0U, buf, sizeof(buf)) == RTE_STATUS_NOT_SUPPORTED);
    assert(rte_nvm_write(handle, 0U, buf, sizeof(buf)) == RTE_STATUS_NOT_SUPPORTED);
    ...
    assert(rte_nvm_read(handle, 0U, buf, sizeof(buf)) == RTE_STATUS_OK);
    assert(g_mock_read_calls == 1);

    assert(rte_nvm_write(handle, 0U, buf, sizeof(buf)) == RTE_STATUS_OK);
    assert(g_mock_write_calls == 1);

    assert(rte_nvm_sync(handle) == RTE_STATUS_OK);
    assert(g_mock_sync_calls == 1);

    assert(rte_nvm_close(handle) == RTE_STATUS_OK);
