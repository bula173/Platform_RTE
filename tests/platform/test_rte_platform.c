...
static const rte_platform_backend_t g_mock_backend __attribute__((unused)) = { mock_realtime_init };
static const rte_platform_backend_t g_mock_backend_empty __attribute__((unused)) = { NULL };
...
int main(void)
{
    ...
    assert(rte_platform_register_backend(NULL) == RTE_STATUS_INVALID_PARAM);

    /* Backend registered but the vtable slot is NULL. */
    assert(rte_platform_register_backend(&g_mock_backend_empty) == RTE_STATUS_OK);
    assert(rte_platform_realtime_init(80U) == RTE_STATUS_NOT_SUPPORTED);

    /* Real dispatch. */
    assert(rte_platform_register_backend(&g_mock_backend) == RTE_STATUS_OK);
