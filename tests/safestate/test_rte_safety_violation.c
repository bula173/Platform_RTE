/* Tests for rte_safety_violation - the opt-in notification hook shared by
 * the three new safety primitives (safe pointer, checked cast arithmetic,
 * bounds check). Unlike rte_safestate's handler (which never returns),
 * this handler is documented to return normally, so no setjmp/longjmp
 * diversion is needed here (contrast test_rte_safestate.c).
 */
#include <assert.h>
#include <string.h>
...
static void capturing_handler(rte_safety_violation_kind_t kind,
                               const char *file,
                               int32_t line,
                               const char *message)
{
...
{
    g_captured_kind = RTE_SAFETY_VIOLATION_CORRUPTION;
...
{
    assert(rte_safety_violation_register_handler(NULL) == RTE_STATUS_INVALID_PARAM);
}
...
static void test_report_is_noop_with_no_handler_registered(void)
{
    ...
    rte_safety_violation_report(RTE_SAFETY_VIOLATION_OUT_OF_RANGE, __FILE__, __LINE__, "no handler yet");
}

static void test_registered_handler_is_invoked_with_correct_args(void)
{
    reset_capture();
    assert(rte_safety_violation_register_handler(capturing_handler) == RTE_STATUS_OK);

    rte_safety_violation_report(RTE_SAFETY_VIOLATION_OVERFLOW, "some_file.c", 42, "overflow detail");

    assert(g_handler_calls == 1);
    assert(g_captured_kind == RTE_SAFETY_VIOLATION_OVERFLOW);
...
static void test_registering_again_replaces_previous_handler(void)
{
    reset_capture();
    assert(rte_safety_violation_register_handler(capturing_handler) == RTE_STATUS_OK);
    rte_safety_violation_report(RTE_SAFETY_VIOLATION_CORRUPTION, __FILE__, __LINE__, "first");
    assert(g_handler_calls == 1);

    /* A second registration must replace, not stack, the handler - same
     * convention as rte_safestate_register_handler(). */
    assert(rte_safety_violation_register_handler(capturing_handler) == RTE_STATUS_OK);
    rte_safety_violation_report(RTE_SAFETY_VIOLATION_CORRUPTION, __FILE__, __LINE__, "second");
