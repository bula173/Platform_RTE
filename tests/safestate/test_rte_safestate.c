/* Tests for rte_safestate (ADR-004).
 *
 * RTE_SAFESTATE_LEVEL_SAFE and RTE_SAFESTATE_LEVEL_REBOOT are documented
 ...
 * divert control flow away from rte_safestate_enter() the same way a real
...
__attribute__((unused))
static void returning_handler(rte_safestate_level_t level __attribute__((unused)),
                               rte_safestate_reason_t reason __attribute__((unused)),
...
{
    assert(rte_safestate_register_handler(RTE_SAFESTATE_LEVEL_SAFE, NULL)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_safestate_register_handler((rte_safestate_level_t)99, diverting_handler)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_safestate_register_handler(RTE_SAFESTATE_LEVEL_DEGRADED, returning_handler)
           == RTE_STATUS_OK);
}
...
{
    g_degraded_calls = 0;
    RTE_SAFESTATE(RTE_SAFESTATE_LEVEL_DEGRADED, 7U);
    assert(g_degraded_calls == 1);
    assert(g_captured_level == RTE_SAFESTATE_LEVEL_DEGRADED);
...
{
    assert(rte_safestate_register_handler(RTE_SAFESTATE_LEVEL_SAFE, diverting_handler)
           == RTE_STATUS_OK);

    g_handler_calls = 0;
    if (setjmp(g_jmp) == 0)
    {
        RTE_SAFESTATE(RTE_SAFESTATE_LEVEL_SAFE, 42U);
...
{
    assert(rte_safestate_register_handler(RTE_SAFESTATE_LEVEL_REBOOT, diverting_handler)
           == RTE_STATUS_OK);
...
static void test_assert_macro(void)
{
    ...
    /* True condition: RTE_ASSERT does nothing, control returns normally. */
    RTE_ASSERT(1 == 1);
    ...
    else
    {
        assert(g_handler_calls == 1);
        assert(g_captured_level == RTE_SAFESTATE_LEVEL_SAFE);
        assert(g_captured_reason == RTE_SAFESTATE_REASON_ASSERT_FAILED);
        assert(g_captured_message != NULL);
...
static void test_unrecognized_level_halts_forever(void)
{
    ...
    if (sigsetjmp(g_sigjmp, 1) == 0)
    {
        ...
        rte_safestate_enter((rte_safestate_level_t)99, 0U, __FILE__, __LINE__, NULL);
