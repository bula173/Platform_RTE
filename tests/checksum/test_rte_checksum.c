 ...
 * initialized" branches of rte_checksum_crc64() from a single test
...
static void child_test_polynomial(rte_crc64_polynomial_t polynomial)
{
    ...
    crc_a = rte_checksum_crc64(data, sizeof(data));
    crc_b = rte_checksum_crc64(data, sizeof(data));
    assert(crc_a == crc_b);
...
static void run_polynomial_in_child(rte_crc64_polynomial_t polynomial)
{
    pid_t pid;
    int wstatus = 0;
...
static void make_message(rte_vital_message_t *msg_out,
                          uint32_t sender_id,
                          uint32_t sequence,
                          const uint8_t *payload,
                          size_t payload_size)
{
    assert(rte_checksum_vital_message_create(msg_out, sender_id, sequence,
                                               payload, payload_size) == RTE_STATUS_OK);
}
...
static void test_crc64_init_invalid_polynomial(void)
{
    ...
    assert(rte_checksum_crc64_init((rte_crc64_polynomial_t)99) == RTE_STATUS_INVALID_PARAM);
}

static void test_crc64_polynomials_in_children(void)
{
    run_polynomial_in_child(RTE_CRC64_ISO);
    run_polynomial_in_child(RTE_CRC64_XZ);
}
...
static void test_crc64_init_real_and_already_initialized(void)
{
    ...
    assert(rte_checksum_crc64_get_polynomial() == RTE_CRC64_ERTMS);

    /* REQ-CHECKSUM-001: second call must fail, table/polynomial
     * unchanged. */
    assert(rte_checksum_crc64_init(RTE_CRC64_ISO) == RTE_STATUS_ALREADY_INITIALIZED);
    assert(rte_checksum_crc64_get_polynomial() == RTE_CRC64_ERTMS);
}
...
static void test_crc64_null_and_empty_data(void)
{
    ...
    assert(rte_checksum_crc64(NULL, 0U) == 0xFFFFFFFFFFFFFFFFULL);

    /* data == NULL, size != 0: must not dereference data - returns 0
     * (REQ-CHECKSUM-002). */
    assert(rte_checksum_crc64(NULL, 5U) == 0ULL);
}

static void test_crc64_normal_and_stats(void)
{
    const uint8_t data[8] = { 0xDEU, 0xADU, 0xBEU, 0xEFU, 0x00U, 0x11U, 0x22U, 0x33U };
    rte_checksum_stats_t before;
    rte_checksum_stats_t after;
    rte_crc64_t crc1;
    rte_crc64_t crc2;

    assert(rte_checksum_get_stats(&before) == RTE_STATUS_OK);

    /* Non-empty buffer: normal loop path. */
    crc1 = rte_checksum_crc64(data, sizeof(data));
    assert(crc1 != 0ULL);
    ...
    crc2 = rte_checksum_crc64(data, 0U);
    assert(crc2 == 0ULL);

    assert(rte_checksum_get_stats(&after) == RTE_STATUS_OK);
...
static void test_crc64_verify(void)
{
    ...
    rte_crc64_t good_crc;
    rte_checksum_result_t result;
    rte_checksum_stats_t before;
    rte_checksum_stats_t after;

    /* NULL result_out -> INVALID_PARAM. */
    assert(rte_checksum_crc64_verify(data, sizeof(data), 0ULL, NULL) == RTE_STATUS_INVALID_PARAM);

    good_crc = rte_checksum_crc64(data, sizeof(data));

    /* Matching CRC -> OK, verification_passes incremented. */
    assert(rte_checksum_get_stats(&before) == RTE_STATUS_OK);
    assert(rte_checksum_crc64_verify(data, sizeof(data), good_crc, &result) == RTE_STATUS_OK);
     ...
     * incremented. */
    assert(rte_checksum_get_stats(&before) == RTE_STATUS_OK);
    assert(rte_checksum_crc64_verify(data, sizeof(data), good_crc ^ 0x1ULL, &result)
...
static void test_vital_message_create_param_validation(void)
{
    ...
    assert(rte_checksum_vital_message_create(NULL, 1U, 1U, big_payload, 4U)
           == RTE_STATUS_INVALID_PARAM);

    /* payload_size > 248 -> INVALID_PARAM, payload_oversize incremented. */
    assert(rte_checksum_get_stats(&before) == RTE_STATUS_OK);
    assert(rte_checksum_vital_message_create(&msg, 1U, 1U, big_payload, sizeof(big_payload))
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_checksum_get_stats(&after) == RTE_STATUS_OK);
    assert(after.payload_oversize == before.payload_oversize + 1U);

    /* payload == NULL && payload_size != 0 -> INVALID_PARAM. */
    assert(rte_checksum_vital_message_create(&msg, 1U, 1U, NULL, 4U)
           == RTE_STATUS_INVALID_PARAM);
}
...
static void test_vital_message_create_and_verify_roundtrip(void)
{
    ...
    assert(rte_checksum_vital_message_verify(&msg, 42U, out_payload, sizeof(out_payload), &out_size)
           == RTE_STATUS_OK);
...
static void test_vital_message_create_empty_payload(void)
{
    ...
    assert(rte_checksum_vital_message_verify(&msg, 7U, out_payload, sizeof(out_payload), &out_size)
           == RTE_STATUS_OK);
...
static void test_vital_message_verify_param_validation(void)
{
    ...
    assert(rte_checksum_vital_message_verify(NULL, 1U, out_payload, sizeof(out_payload), &out_size)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_checksum_vital_message_verify(&msg, 1U, NULL, sizeof(out_payload), &out_size)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_checksum_vital_message_verify(&msg, 1U, out_payload, sizeof(out_payload), NULL)
           == RTE_STATUS_INVALID_PARAM);
}
...
static void test_vital_message_verify_corrupted_crc(void)
{
    ...
    assert(rte_checksum_vital_message_verify(&msg, 5U, out_payload, sizeof(out_payload), &out_size)
           == RTE_STATUS_DATA_CORRUPTION);
}
...
static void test_vital_message_verify_sequence_mismatch(void)
{
    ...
    assert(rte_checksum_get_stats(&before) == RTE_STATUS_OK);
    assert(rte_checksum_vital_message_verify(&msg, 11U /* wrong */, out_payload,
...
static void test_vital_message_verify_oversized_payload(void)
{
    ...
    assert(rte_checksum_get_stats(&before) == RTE_STATUS_OK);
    assert(rte_checksum_vital_message_verify(&msg, 20U, out_payload, sizeof(out_payload), &out_size)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_checksum_get_stats(&after) == RTE_STATUS_OK);
    assert(after.payload_oversize == before.payload_oversize + 1U);
}
...
{
    assert(rte_checksum_get_stats(NULL) == RTE_STATUS_INVALID_PARAM);
}
...
static void test_reset_stats(void)
{
    ...
    assert(rte_checksum_reset_stats() == RTE_STATUS_OK);
