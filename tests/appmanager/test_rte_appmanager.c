/* Tests for rte_appmanager (previously untested): lifecycle ordering,
 * max_iterations/error_threshold bounds, rte_appmanager_request_shutdown(),
 * and the POSIX-only rte_appmanager_install_default_signal_handlers()
 * convenience (see that function's own doc in rte_appmanager.h for why
 ...
 * tests/checkpoint/test_rte_checkpoint.c, for the same reason: a
 * checkpoint timeout calls rte_safestate_enter() at
 * RTE_SAFESTATE_LEVEL_SAFE, which this framework's shipped
 * rte_safestate.c halts permanently (REQ-COMMON-SAFESTATE-002) - the
...
static rte_status_t fake_init_ok(void *context)
{
    (void)context;
    g_init_calls++;
...
static rte_status_t fake_init_fail(void *context)
{
    (void)context;
    g_init_calls++;
...
static rte_status_t fake_execute_ok(void *context)
{
    (void)context;
    g_execute_calls++;
    g_last_seen_state_in_execute = (int)rte_appmanager_get_state();
    return RTE_STATUS_OK;
}

static rte_status_t fake_execute_always_fails(void *context)
{
    (void)context;
    g_execute_calls++;
    return RTE_STATUS_INTERNAL_ERROR;
...
static rte_status_t fake_execute_requests_shutdown_after_3(void *context)
{
    (void)context;
    g_execute_calls++;
    if (g_execute_calls >= 3)
    {
        rte_appmanager_request_shutdown();
    }
    return RTE_STATUS_OK;
}

static rte_status_t fake_shutdown(void *context)
{
    (void)context;
    g_shutdown_calls++;
    return RTE_STATUS_OK;
...
static rte_status_t fake_shutdown_fails(void *context)
{
    (void)context;
    g_shutdown_calls++;
    return RTE_STATUS_INTERNAL_ERROR;
...
static void test_config_validation(void)
{
    ...
    static const rte_appmanager_operations_t incomplete_ops = {
        .init = NULL,
    ...
    assert(rte_appmanager_run(NULL) == EXIT_FAILURE);
    ...
    assert(rte_appmanager_run(&config) == EXIT_FAILURE);
...
{
    static const rte_appmanager_operations_t ops = {
        .init = fake_init_fail,
        .execute = fake_execute_ok,
        .shutdown = fake_shutdown,
        .get_name = fake_get_name,
...
static void test_bounded_run_calls_in_order_with_correct_counts(void)
{
    ...
    assert(rte_appmanager_run(&config) == EXIT_SUCCESS);
    assert(g_init_calls == 1);
    assert(g_execute_calls == 4); /* stopped exactly at max_iterations */
    assert(g_shutdown_calls == 1);
    assert(g_last_seen_state_in_execute == (int)RTE_APP_STATE_RUNNING);
    assert(rte_appmanager_get_state() == RTE_APP_STATE_SHUTDOWN);
}

/* A non-OK shutdown() status is logged (stderr) but never fails the run
 * overall - rte_appmanager_run()'s own comment says "Continue anyway;
...
static void test_error_threshold_stops_run_early(void)
{
    ...
    assert(rte_appmanager_get_stats(&stats) == RTE_STATUS_OK);
    assert(stats.error_count == 3U);
    assert(stats.last_error == RTE_STATUS_INTERNAL_ERROR);

    assert(rte_appmanager_get_stats(NULL) == RTE_STATUS_INVALID_PARAM);
}
...
static rte_status_t fake_execute_raises_sigint_once(void *context)
{
    (void)context;
...
static void test_install_default_signal_handlers(void)
{
    ...
    assert(rte_appmanager_install_default_signal_handlers() == RTE_STATUS_OK);
     ...
     * rte_appmanager.c), so the run ends after that 1st iteration
...
static rte_status_t hook_on_checkpoint_result_tracks(void *context, bool committed)
{
    (void)context;
...
static rte_status_t hook_pre_ok(void *context)
{
    (void)context;
    g_pre_calls++;
...
static rte_status_t hook_execute_ok(void *context)
{
    (void)context;
    g_execute_calls++;
...
static rte_status_t hook_post_ok(void *context)
{
    (void)context;
    g_post_calls++;
...
static rte_status_t hook_pre_fails(void *context)
{
    (void)context;
    g_pre_calls++;
    if (g_hook_order_len < TEST_HOOK_ORDER_MAX) { g_hook_order[g_hook_order_len++] = 'p'; }
...
static rte_status_t hook_execute_fails(void *context)
{
    (void)context;
    g_execute_calls++;
    if (g_hook_order_len < TEST_HOOK_ORDER_MAX) { g_hook_order[g_hook_order_len++] = 'e'; }
 ...
 * tests/checkpoint/test_rte_checkpoint.c: backend_send()/backend_recv()
...
static rte_vital_message_t g_cp_mailbox[TEST_CP_CHANNEL_COUNT];
static bool g_cp_reply_ready[TEST_CP_CHANNEL_COUNT];

static jmp_buf g_cp_jmp;
static rte_safestate_level_t g_cp_captured_level;
static rte_safestate_reason_t g_cp_captured_reason;
static int g_cp_handler_calls;

static void cp_diverting_handler(rte_safestate_level_t level, rte_safestate_reason_t reason,
...
static rte_status_t cp_mock_backend_send(void *channel, const void *data, size_t data_size)
{
...
static rte_status_t cp_mock_backend_recv(void *channel, void *data, size_t data_size, uint32_t timeout_ms)
{
    ...
    (void)memcpy(data, &g_cp_mailbox[idx], sizeof(rte_vital_message_t));
    return RTE_STATUS_OK;
}
...
static void cp_seed_valid_reply(int idx, uint32_t checkpoint_id)
{
    ...
    assert(rte_checksum_vital_message_create(&g_cp_mailbox[idx], 0xAAU, checkpoint_id, payload, sizeof(payload))
           == RTE_STATUS_OK);
...
static void cp_init_voter(rte_channel_t channels[TEST_CP_CHANNEL_COUNT], rte_voter_t *voter)
{
...
/* Replicates rte_appmanager_checkpoint_mark()'s own fold algorithm
 ...
 * same public rte_checksum_crc64()) rather than just trusting it.
 * Deliberately kept in lockstep with rte_appmanager.c's own (private)
...
{
    uint8_t buf[16];
    rte_crc64_t mark_hash = rte_checksum_crc64((const uint8_t *)text, strlen(text));
    size_t i;
    ...
    return (uint64_t)rte_checksum_crc64(buf, sizeof(buf));
}

static void test_checkpoint_fold_signature_is_deterministic(void)
{
    uint32_t seed_fold_a = rte_appmanager_checkpoint_fold_signature(RTE_APPMANAGER_CHECKPOINT_SIGNATURE_SEED);
    uint32_t seed_fold_b = rte_appmanager_checkpoint_fold_signature(RTE_APPMANAGER_CHECKPOINT_SIGNATURE_SEED);
    uint32_t other_fold = rte_appmanager_checkpoint_fold_signature(0x1234567890ABCDEFULL);
...
static void test_checkpoint_mark_sequence_is_order_sensitive(void)
{
    uint64_t after_a_then_b = cp_replicate_mark_fold(
        cp_replicate_mark_fold(RTE_APPMANAGER_CHECKPOINT_SIGNATURE_SEED, "mark-a"), "mark-b");
    uint64_t after_b_then_a = cp_replicate_mark_fold(
        cp_replicate_mark_fold(RTE_APPMANAGER_CHECKPOINT_SIGNATURE_SEED, "mark-b"), "mark-a");
    uint64_t after_a_only = cp_replicate_mark_fold(RTE_APPMANAGER_CHECKPOINT_SIGNATURE_SEED, "mark-a");
    ...
    assert(rte_appmanager_checkpoint_fold_signature(after_a_then_b)
           != rte_appmanager_checkpoint_fold_signature(after_b_then_a));
    assert(rte_appmanager_checkpoint_fold_signature(after_a_then_b)
           != rte_appmanager_checkpoint_fold_signature(after_a_only));
}

static rte_status_t hook_execute_marks_a_then_b(void *context)
{
    (void)context;
    g_execute_calls++;
    RTE_CHECKPOINT_MARK_LABEL("mark-a");
    RTE_CHECKPOINT_MARK_LABEL("mark-b");
    return RTE_STATUS_OK;
}

static void test_checkpoint_success_runs_after_post_execute_each_cycle(void)
{
    rte_channel_t vc[TEST_CP_CHANNEL_COUNT];
    rte_voter_t voter;
    rte_appmanager_checkpoint_config_t cp_cfg;
    rte_appmanager_config_t config;
    ...
    expected_id = rte_appmanager_checkpoint_fold_signature(
 ...
 * feature into safeAPIRBC2oo2's channel_ab.c: rte_appmanager_run() must
 ...
 * rte_voter_init()-ed until application startup), which runs AFTER
 ...
 * check, including rte_watchdog_timer_tick() - never ran for as long as
...
static void test_checkpoint_timeout_diverts_after_cycle_runs_once(void)
{
    ...
    assert(rte_safestate_register_handler(RTE_SAFESTATE_LEVEL_SAFE, cp_diverting_handler) == RTE_STATUS_OK);

    g_cp_handler_calls = 0;
    if (setjmp(g_cp_jmp) == 0)
    {
        (void)rte_appmanager_run(&config);
        /* Must never reach here: an unconfirmed checkpoint always
         * diverts via longjmp before rte_appmanager_run() can return. */
        assert(0);
    }
    else
    {
        assert(g_cp_handler_calls == 1);
        assert(g_cp_captured_level == RTE_SAFESTATE_LEVEL_SAFE);
        assert(g_cp_captured_reason == RTE_SAFESTATE_REASON_CHECKPOINT_TIMEOUT);
         ...
         * means rte_appmanager_run() above never reached its own
         * SHUTDOWN phase, so its internal lifecycle state (and the
         * ADR-026 setup-phase lock) is left stuck at RUNNING/locked.
         * See rte_appmanager_reset_state()'s own doc - this is exactly
         * the case it exists for. */
        rte_appmanager_reset_state();
    }
}
 ...
 *  rte_appmanager_pace_failed_checkpoint() mechanism it exercised) - that
...
static void test_checkpoint_paused_still_calls_on_checkpoint_result_committed(void)
{
    ...
    cp_cfg.voter = NULL; /* paused - must be skipped outright, not fed to rte_channel_checkpoint() */
    cp_cfg.max_delay_ms = 20U;
    cp_cfg.expected_node_count = 1U;
    cp_cfg.watchdog = NULL;
