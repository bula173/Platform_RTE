/* Tests for rte_dual_channel_t (ADR-020 Layer 2): DATA send with
 ...
 * by rte_dual_negotiator_t.
 *
 * Follows tests/checkpoint/test_rte_checkpoint.c's own precedent for
 ...
 * blocking wait, and a fresh rte_dual_channel_send() call always
 ...
 * rte_dual_msgchannel_t standing in for "what the peer would have
 ...
 *  rte_dual_channel_send()'s own ACK-wait loop through its
...
static void init_channel_a(fixture_t *fx, rte_dual_channel_t *channel, rte_dual_channel_status_callback_t cb)
{
    rte_dual_channel_config_t cfg;

    (void)memset(&cfg, 0, sizeof(cfg));
    cfg.links[0]         = (rte_netlink_handle_t)&fx->a_link0;
    cfg.links[1]         = (rte_netlink_handle_t)&fx->a_link1;
    cfg.link_count       = 2U;
    cfg.sender_id        = 1U;
...
static void init_channel_b(fixture_t *fx, rte_dual_channel_t *channel)
{
    rte_dual_channel_config_t cfg;

    (void)memset(&cfg, 0, sizeof(cfg));
    cfg.links[0]         = (rte_netlink_handle_t)&fx->b_link0;
    cfg.links[1]         = (rte_netlink_handle_t)&fx->b_link1;
    cfg.link_count       = 2U;
    cfg.sender_id        = 2U;
...
/** Seeds a valid RTE_DUAL_FRAME_KIND_ACK frame, as "B" would have sent
...
static void seed_ack(mock_link_t *b_side_link, uint32_t acked_sequence)
{
    ...
    seed_cfg.link             = (rte_netlink_handle_t)b_side_link;
...
/** Seeds a raw RTE_DUAL_FRAME_KIND_DATA frame, as "A" would have sent
 *  it, directly into a mailbox. */
static void seed_data(mock_link_t *a_side_link, const uint8_t *payload, uint8_t payload_size)
{
    rte_dual_msgchannel_t        seed;
    rte_dual_msgchannel_config_t seed_cfg;
    uint8_t                        frame[RTE_DUAL_CHANNEL_MAX_PAYLOAD + 4U];
    rte_dual_frame_header_t       header;

    seed_cfg.link             = (rte_netlink_handle_t)a_side_link;
 ...
 *  kind frames that rte_dual_msgchannel_send()'s own higher-level
...
static void seed_raw(mock_link_t *b_side_link, const uint8_t *raw, uint8_t raw_size)
{
    ...
    assert(rte_dual_msgchannel_send(&seed, raw, raw_size, 10U, NULL) == RTE_STATUS_OK);
}

static int                        g_status_callback_calls = 0;
static rte_dual_channel_status_t g_last_new_status;
static rte_dual_channel_status_t g_last_old_status;

static void status_callback(rte_dual_channel_status_t new_status, rte_dual_channel_status_t old_status,
                             void *user_ctx)
{
    (void)user_ctx;
...
static void test_invalid_params(void)
{
    ...
    assert(rte_dual_channel_init(NULL, &cfg) == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_init(&channel_a, NULL) == RTE_STATUS_INVALID_PARAM);

    {
        rte_dual_channel_config_t zero_links = cfg;

        zero_links.link_count = 0U;
        assert(rte_dual_channel_init(&channel_a, &zero_links) == RTE_STATUS_INVALID_PARAM);
    }
    {
        rte_dual_channel_config_t too_many = cfg;

        too_many.link_count = RTE_DUAL_CHANNEL_MAX_LINKS + 1U;
        assert(rte_dual_channel_init(&channel_a, &too_many) == RTE_STATUS_INVALID_PARAM);
    }
    {
        /* cfg.links[1] is still NULL (fixture only set links[0] above)
         * with link_count claiming 2 populated entries. */
        assert(rte_dual_channel_init(&channel_a, &cfg) == RTE_STATUS_INVALID_PARAM);
    }
}
...
static void test_send_both_links_ack_full(void)
{
    ...
    assert(rte_dual_channel_get_status(&channel_a) == RTE_DUAL_CHANNEL_STATUS_DOWN);

    seed_ack(&fx.b_link0, 0U);
    seed_ack(&fx.b_link1, 0U);

    assert(rte_dual_channel_send(&channel_a, (const uint8_t *)"AB_SAMPLE", 9U, &ack_count) == RTE_STATUS_OK);
    assert(ack_count == 2U);
    assert(rte_dual_channel_get_status(&channel_a) == RTE_DUAL_CHANNEL_STATUS_FULL);
    assert(rte_dual_channel_is_link_up(&channel_a, 0U) == true);
    assert(rte_dual_channel_is_link_up(&channel_a, 1U) == true);
    assert(g_status_callback_calls == 1);
    assert(g_last_old_status == RTE_DUAL_CHANNEL_STATUS_DOWN);
    assert(g_last_new_status == RTE_DUAL_CHANNEL_STATUS_FULL);
}
...
static void test_send_one_link_acks_degraded(void)
{
    ...
    assert(rte_dual_channel_send(&channel_a, (const uint8_t *)"x", 1U, &ack_count) == RTE_STATUS_OK);
    assert(ack_count == 1U);
    assert(rte_dual_channel_get_status(&channel_a) == RTE_DUAL_CHANNEL_STATUS_DEGRADED);
    assert(rte_dual_channel_is_link_up(&channel_a, 0U) == true);
    assert(rte_dual_channel_is_link_up(&channel_a, 1U) == false);
    assert(g_last_new_status == RTE_DUAL_CHANNEL_STATUS_DEGRADED);
}
...
static void test_send_no_acks_down(void)
{
    ...
    assert(rte_dual_channel_send(&channel_a, (const uint8_t *)"y", 1U, &ack_count) == RTE_STATUS_TIMEOUT);
    assert(ack_count == 0U);
    assert(rte_dual_channel_get_status(&channel_a) == RTE_DUAL_CHANNEL_STATUS_DOWN);
}
...
static void test_send_broken_link_never_counts(void)
{
    ...
    assert(rte_dual_channel_send(&channel_a, (const uint8_t *)"z", 1U, &ack_count) == RTE_STATUS_OK);
    assert(ack_count == 1U);
    assert(rte_dual_channel_is_link_up(&channel_a, 0U) == true);
    assert(rte_dual_channel_is_link_up(&channel_a, 1U) == false);
}
 ...
 *  fails (peer closed/reset the connection - RTE_STATUS_HARDWARE_FAULT,
 *  not just "hasn't ACKed yet") must not be silently collapsed into the
 *  same RTE_STATUS_TIMEOUT test_send_no_acks_down() above exercises for
...
static void test_send_hard_fault_propagates_status(void)
{
    ...
    assert(rte_dual_channel_send(&channel_a, (const uint8_t *)"y", 1U, &ack_count) == RTE_STATUS_HARDWARE_FAULT);
    assert(ack_count == 0U);
    assert(rte_dual_channel_get_status(&channel_a) == RTE_DUAL_CHANNEL_STATUS_DOWN);
}
 ...
 *  rte_dual_channel_receive() sweeps it must not be silently collapsed
 *  into the same RTE_STATUS_TIMEOUT "nothing staged yet" every other
 *  empty-sweep case returns. */
static void test_receive_hard_fault_propagates_status(void)
{
    fixture_t            fx;
    rte_dual_channel_t   channel_a;
    uint8_t               out_payload[RTE_DUAL_CHANNEL_MAX_PAYLOAD];
    uint8_t               out_size = 0U;
    ...
    assert(rte_dual_channel_receive(&channel_a, out_payload, sizeof(out_payload), 5U, &out_size)
...
static void test_receive_auto_acks_and_stages_payload(void)
{
    ...
    assert(rte_dual_channel_receive(&channel_b, out_payload, sizeof(out_payload), 5U, &out_size) == RTE_STATUS_OK);
    assert(out_size == 5U);
    ...
    {
        rte_dual_msgchannel_t        checker;
        rte_dual_msgchannel_config_t checker_cfg;
        uint8_t                       raw[RTE_DUAL_MSGCHANNEL_MAX_PAYLOAD];
        uint8_t                       raw_size = 0U;
        rte_dual_ack_frame_t          ack;

        checker_cfg.link             = (rte_netlink_handle_t)&fx.a_link0;
        checker_cfg.sender_id        = 1U;
        checker_cfg.expected_peer_id = 2U;
        assert(rte_dual_msgchannel_init(&checker, &checker_cfg) == RTE_STATUS_OK);
        assert(rte_dual_msgchannel_receive(&checker, raw, sizeof(raw), 5U, &raw_size, NULL) == RTE_STATUS_OK);
        assert(raw_size >= (uint8_t)sizeof(ack));
        (void)memcpy(&ack, raw, sizeof(ack));
        assert(ack.header.kind == (uint8_t)RTE_DUAL_FRAME_KIND_ACK);
        assert(ack.acked_sequence == 0U);
    }

    /* Nothing else pending. */
    assert(rte_dual_channel_receive(&channel_b, out_payload, sizeof(out_payload), 0U, &out_size)
           == RTE_STATUS_TIMEOUT);
}
...
static void test_state_frame_roundtrip(void)
{
    ...
    rte_dual_channel_t       channel_a;
    rte_dual_channel_t       channel_b;
    rte_dual_state_frame_t   received;

    fixture_init(&fx);
    init_channel_a(&fx, &channel_a, NULL);
    init_channel_b(&fx, &channel_b);

    assert(rte_dual_channel_send_state_frame(&channel_a, RTE_DUAL_STATE_ONLINE, false, 12345U) == RTE_STATUS_OK);
     ...
     * was processed last - see rte_dual_channel_receive()'s own
     * "sweeps every link" doc, which _receive_state_frame() shares. */
    assert(rte_dual_channel_receive_state_frame(&channel_b, 5U, &received) == RTE_STATUS_OK);
    assert(received.state == (uint8_t)RTE_DUAL_STATE_ONLINE);
    ...
    assert(rte_dual_channel_receive_state_frame(&channel_b, 0U, &received) == RTE_STATUS_TIMEOUT);
}
...
static void test_send_param_validation(void)
{
    fixture_t            fx;
    rte_dual_channel_t   channel_a;
    uint8_t               oversize[RTE_DUAL_CHANNEL_MAX_PAYLOAD + 1U];

    fixture_init(&fx);
    init_channel_a(&fx, &channel_a, NULL);
    (void)memset(oversize, 0, sizeof(oversize));

    assert(rte_dual_channel_send(NULL, (const uint8_t *)"a", 1U, NULL) == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_send(&channel_a, NULL, 1U, NULL) == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_send(&channel_a, oversize, (uint8_t)sizeof(oversize), NULL) == RTE_STATUS_INVALID_PARAM);
}
...
static void test_receive_param_validation(void)
{
    ...
    assert(rte_dual_channel_receive(NULL, out_payload, sizeof(out_payload), 0U, &out_size)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_receive(&channel_b, NULL, sizeof(out_payload), 0U, &out_size)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_receive(&channel_b, out_payload, sizeof(out_payload), 0U, NULL)
           == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_receive(&channel_b, out_payload, 0U, 0U, &out_size) == RTE_STATUS_INVALID_PARAM);
}
...
static void test_receive_staged_frame_too_big_for_caller_buffer(void)
{
    ...
    seed_data(&fx.a_link0, (const uint8_t *)"hello", 5U);
    assert(rte_dual_channel_receive(&channel_b, small_buf, sizeof(small_buf), 5U, &out_size)
           == RTE_STATUS_INVALID_PARAM);
}
...
static void test_send_state_frame_and_receive_state_frame_param_validation(void)
{
    ...
    assert(rte_dual_channel_send_state_frame(NULL, RTE_DUAL_STATE_ONLINE, false, 1U) == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_receive_state_frame(NULL, 0U, &received) == RTE_STATUS_INVALID_PARAM);
    assert(rte_dual_channel_receive_state_frame(&channel_a, 0U, NULL) == RTE_STATUS_INVALID_PARAM);
}
...
static void test_get_status_and_is_link_up_defensive_defaults(void)
{
    ...
    assert(rte_dual_channel_get_status(NULL) == RTE_DUAL_CHANNEL_STATUS_DOWN);
    assert(rte_dual_channel_is_link_up(NULL, 0U) == false);
    assert(rte_dual_channel_is_link_up(&channel_a, channel_a.link_count) == false);
    assert(rte_dual_channel_is_link_up(&channel_a, 999U) == false);
}
 ...
 *  rte_dual_frame_header_t, a RTE_DUAL_FRAME_KIND_ACK frame too short
 *  to hold its own acked_sequence field, a RTE_DUAL_FRAME_KIND_STATE
 ...
 *  rte_dual_msgchannel_send()'s own normal helpers only ever build
 *  well-formed frames. None of these are ever surfaced back through the
 *  public rte_dual_channel_receive() API (its own sweep loop discards
...
static void test_receive_rejects_malformed_and_unknown_frames(void)
{
    ...
    /* Kind == ACK but shorter than sizeof(rte_dual_ack_frame_t) (8 bytes). */
    {
        uint8_t short_ack[5] = { (uint8_t)RTE_DUAL_FRAME_KIND_ACK, 0U, 0U, 0U, 0U };
    ...
    /* Kind == STATE but shorter than sizeof(rte_dual_state_frame_t). */
    {
        uint8_t short_state[6] = { (uint8_t)RTE_DUAL_FRAME_KIND_STATE, 0U, 0U, 0U, 0U, 0U };
...
/** Exercises rte_dual_channel_send()'s own ACK-wait loop actually
 ...
 *  rte_timer_now() call), a link with no ACK ever seeded runs the
...
static void test_send_ack_wait_observes_timer_progress(void)
{
     ...
     * cap. */
    assert(rte_dual_channel_send(&channel_a, (const uint8_t *)"t", 1U, &ack_count) == RTE_STATUS_TIMEOUT);
    assert(ack_count == 0U);
    assert(rte_dual_channel_is_link_up(&channel_a, 0U) == false);
    assert(rte_dual_channel_is_link_up(&channel_a, 1U) == false);
}

/** Exercises rte_dual_channel_send()'s own ACK-wait loop's
...
static void test_send_ack_wait_handles_stalled_clock(void)
{
    ...
    assert(rte_dual_channel_send(&channel_a, (const uint8_t *)"s", 1U, &ack_count) == RTE_STATUS_TIMEOUT);
    assert(ack_count == 0U);
...
static void test_send_heartbeat(void)
{
    ...
    assert(rte_dual_channel_send_heartbeat(NULL, &ack_count) == RTE_STATUS_INVALID_PARAM);

    /* Both links timeout when nothing seeded */
    assert(rte_dual_channel_send_heartbeat(&channel_a, &ack_count) == RTE_STATUS_TIMEOUT);
    assert(ack_count == 0U);
    assert(rte_dual_channel_get_status(&channel_a) == RTE_DUAL_CHANNEL_STATUS_DOWN);
    ...
    assert(rte_dual_channel_send_heartbeat(&channel_a, &ack_count) == RTE_STATUS_OK);
    assert(ack_count == 2U);
    assert(rte_dual_channel_get_status(&channel_a) == RTE_DUAL_CHANNEL_STATUS_FULL);
}
