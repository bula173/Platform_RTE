/* Tests for rte_cross_comparator (ADR-025): 2-way comparison between
 * registered rte_channel_t links. */
#include <assert.h>
#include <setjmp.h>
#include <string.h>
...
static void reset_mocks(void)
{
    ...
    g_mock[1].recv_status = RTE_STATUS_OK;
}
...
static rte_voting_result_t g_last_result;
...
{
    rte_cross_comparator_storage_t storage;
    rte_cross_comparator_config_t cfg;
    rte_channel_storage_t ch[3];

    memset(&cfg, 0, sizeof(cfg));

    assert(rte_cross_comparator_init(NULL, &cfg) == RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_init(&storage, NULL) == RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_init(&storage, &cfg) == RTE_STATUS_OK);

    reset_mocks();
    init_channel(&ch[0], 0U);
    init_channel(&ch[1], 1U);

    assert(rte_cross_comparator_register_channel(NULL, &ch[0]) == RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_register_channel(&storage, NULL) == RTE_STATUS_INVALID_PARAM);

    {
        rte_cross_comparator_storage_t not_init;
        memset(&not_init, 0, sizeof(not_init));
        assert(rte_cross_comparator_register_channel(&not_init, &ch[0]) == RTE_STATUS_NOT_INITIALIZED);
    }

    assert(rte_cross_comparator_register_channel(&storage, &ch[0]) == RTE_STATUS_OK);
    assert(rte_cross_comparator_register_channel(&storage, &ch[1]) == RTE_STATUS_OK);
    /* A 3rd registration is rejected. */
    assert(rte_cross_comparator_register_channel(&storage, &ch[2]) == RTE_STATUS_RESOURCE_EXHAUSTED);
}
...
static void test_execute_validation(void)
{
    rte_cross_comparator_storage_t storage;
    rte_cross_comparator_config_t cfg;
    rte_channel_storage_t ch[1];
    rte_voting_result_t result;

    memset(&cfg, 0, sizeof(cfg));
    assert(rte_cross_comparator_init(&storage, &cfg) == RTE_STATUS_OK);

    assert(rte_cross_comparator_execute(NULL, 4U, &result, NULL, NULL) == RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_execute(&storage, 0U, &result, NULL, NULL) == RTE_STATUS_INVALID_PARAM);
    /* Only 0 channels registered so far. */
    assert(rte_cross_comparator_execute(&storage, 4U, &result, NULL, NULL) == RTE_STATUS_INVALID_PARAM);
    ...
    {
        uint8_t oversized_size_marker[RTE_CROSS_COMPARATOR_MAX_MESSAGE_SIZE + 1U];
        (void)oversized_size_marker;
        assert(rte_cross_comparator_execute(&storage, RTE_CROSS_COMPARATOR_MAX_MESSAGE_SIZE + 1U, &result, NULL,
                                              NULL)
...
static void test_execute_agreement(void)
{
    ...
    assert(rte_cross_comparator_execute(&storage, sizeof(out_data), &result, out_data, &out_size)
           == RTE_STATUS_OK);
    ...
    assert(rte_cross_comparator_execute(&storage, sizeof(out_data), &result, NULL, NULL) == RTE_STATUS_OK);
    assert(result == RTE_VOTING_AGREED);

    {
        uint32_t healthy = 0U;
        uint32_t disagreements = 0U;
        assert(rte_cross_comparator_get_aggregated_health(&storage, &healthy, &disagreements) == RTE_STATUS_OK);
        assert(healthy == 2U);
        assert(disagreements == 0U);
    }
    assert(rte_cross_comparator_get_aggregated_health(NULL, NULL, NULL) == RTE_STATUS_INVALID_PARAM);
}
...
static void test_execute_disagreement_triggers_safestate(void)
{
    ...
    if (setjmp(g_jmp) == 0)
    {
        (void)rte_cross_comparator_execute(&storage, 4U, NULL, NULL, NULL);
        assert(0);
    }
    else
    {
...
/* Per the RCA/OCORA PI-API compatibility change (see rte_cross_comparator.h's
...
static void test_execute_disagreement_triggers_reboot_level(void)
{
    ...
    else
    {
        ...
        assert(g_last_result == RTE_VOTING_DISAGREED);
        assert(g_handler_calls == 1);
    }

    {
        uint32_t disagreements = 0U;
        assert(rte_cross_comparator_get_aggregated_health(&storage, NULL, &disagreements) == RTE_STATUS_OK);
...
static void test_execute_timeout_and_unhealthy(void)
{
    ...
    assert(rte_cross_comparator_execute(&storage, 4U, &result, NULL, &out_size) == RTE_STATUS_HARDWARE_FAULT);
    ...
    assert(rte_cross_comparator_execute(&storage, 4U, &result, NULL, NULL) == RTE_STATUS_HARDWARE_FAULT);
    assert(result == RTE_VOTING_TIMEOUT);
    g_mock[0].recv_status = RTE_STATUS_OK;

    /* One channel fails with a non-timeout error. */
    g_mock[0].recv_status = RTE_STATUS_HARDWARE_FAULT;
    assert(rte_cross_comparator_execute(&storage, 4U, &result, NULL, NULL) == RTE_STATUS_HARDWARE_FAULT);
    assert(result == RTE_VOTING_INSUFFICIENT_QUORUM);
}
...
static void test_custom_compare_fn(void)
{
    ...
    assert(rte_cross_comparator_execute(&storage, 4U, &result, NULL, NULL) == RTE_STATUS_OK);
    assert(result == RTE_VOTING_AGREED);
}

/* rte_cross_comparator_execute_buffers() (RCA/OCORA Phase 2b - see
 * rte_cross_comparator.h's own doc): no channel registration needed at
...
static void test_execute_buffers_agreement(void)
{
    ...
    /* Deliberately no rte_cross_comparator_register_channel() call. */
    assert(rte_cross_comparator_execute_buffers(&storage, local_buf, peer_buf, sizeof(local_buf), &result,
                                                  out_data, &out_size) == RTE_STATUS_OK);
    assert(result == RTE_VOTING_AGREED);
...
static void test_execute_buffers_disagreement_triggers_safestate(void)
{
    ...
    if (setjmp(g_jmp) == 0)
    {
        (void)rte_cross_comparator_execute_buffers(&storage, local_buf, peer_buf, sizeof(local_buf), NULL, NULL,
                                                     NULL);
        assert(0);
    }
    else
...
static void test_execute_buffers_validation(void)
{
    ...
    rte_cross_comparator_config_t cfg;
    uint8_t buf[4] = {0};
    uint8_t oversized[RTE_CROSS_COMPARATOR_MAX_MESSAGE_SIZE + 1U];

    memset(&cfg, 0, sizeof(cfg));
    assert(rte_cross_comparator_init(&storage, &cfg) == RTE_STATUS_OK);

    assert(rte_cross_comparator_execute_buffers(NULL, buf, buf, sizeof(buf), NULL, NULL, NULL) ==
           RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_execute_buffers(&storage, NULL, buf, sizeof(buf), NULL, NULL, NULL) ==
           RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_execute_buffers(&storage, buf, NULL, sizeof(buf), NULL, NULL, NULL) ==
           RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_execute_buffers(&storage, buf, buf, 0U, NULL, NULL, NULL) ==
           RTE_STATUS_INVALID_PARAM);
    assert(rte_cross_comparator_execute_buffers(&storage, oversized, oversized, sizeof(oversized), NULL, NULL,
                                                  NULL) == RTE_STATUS_RESOURCE_EXHAUSTED);

    {
        rte_cross_comparator_storage_t uninit;
        memset(&uninit, 0, sizeof(uninit));
        assert(rte_cross_comparator_execute_buffers(&uninit, buf, buf, sizeof(buf), NULL, NULL, NULL) ==
               RTE_STATUS_NOT_INITIALIZED);
    }
}

/* REQ-LIFECYCLE-001 (ADR-026): once the application's setup phase is
 * locked, rte_cross_comparator_init()/_register_channel() refuse even
 * with fully-valid arguments. */
static void test_lifecycle_lock(void)
{
    rte_cross_comparator_storage_t storage;
    rte_cross_comparator_config_t cfg;
    rte_channel_storage_t ch;
    ...
    rte_lifecycle_lock();
    assert(rte_cross_comparator_init(&storage, &cfg) == RTE_STATUS_INVALID_STATE);
    assert(rte_cross_comparator_register_channel(&storage, &ch) == RTE_STATUS_INVALID_STATE);
    rte_lifecycle_unlock();
    assert(rte_cross_comparator_register_channel(&storage, &ch) == RTE_STATUS_OK);
}
...
static void test_destroy(void)
{
    ...
    assert(rte_cross_comparator_destroy(NULL) == RTE_STATUS_OK);
    assert(rte_cross_comparator_destroy(&storage) == RTE_STATUS_OK);
}
