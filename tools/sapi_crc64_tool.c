/**
 * @file sapi_crc64_tool.c
 * @brief CLI wrapper around sapi_checksum_crc64() - prints a file's CRC-64
 *        (SAPI_CRC64_ERTMS polynomial, the same one this framework's own
 *        peer-link integrity check already uses - see
 *        app_main_common_init()'s own sapi_checksum_crc64_init() call) as
 *        lowercase hex.
 *
...
#define SAPI_CRC64_TOOL_MAX_FILE_SIZE (64U * 1024U * 1024U)
static uint8_t g_file_buffer[SAPI_CRC64_TOOL_MAX_FILE_SIZE];
...
int main(int argc, char **argv)
{
    ...
    if (feof(fp) == 0)
    {
         ...
         * SAPI_CRC64_TOOL_MAX_FILE_SIZE. Fail loudly rather than checksum
        ...
        (void)fprintf(stderr, "%s: '%s' exceeds the %u MB buffer this tool supports\n",
                       argv[0], argv[1], (unsigned int)(SAPI_CRC64_TOOL_MAX_FILE_SIZE / (1024U * 1024U)));
        (void)fclose(fp);
        return 1;
    }
    (void)fclose(fp);

    status = sapi_checksum_crc64_init(SAPI_CRC64_ERTMS);
    if (status != SAPI_STATUS_OK)
    {
        (void)fprintf(stderr, "%s: sapi_checksum_crc64_init() failed: %s\n", argv[0], sapi_status_to_string(status));
        return 1;
    }

    crc = sapi_checksum_crc64(g_file_buffer, bytes_read);
