...
Status: Accepted
Date: 2026-08-06
Applies to: `rte_appmanager` (extended, backward-compatible), reusing
`rte_checkpoint`/`rte_channel` (ADR-017) without changes to either.
...
1. A dual-channel application wants to know, every cycle, "is my peer at
   ...
   is what `rte_channel`'s voting already does), but as a guard
   *before* that cycle's decision logic runs. `rte_checkpoint`
   (ADR-017) already does exactly this — `rte_channel_checkpoint()` is a
...
This ADR extends `rte_appmanager` to cover both, without introducing any
new synchronization mechanism — it wires the existing `rte_checkpoint`
...
`rte_appmanager_operations_t` gains two new **optional** members,
...
typedef struct {
    ...
    rte_status_t (*pre_execute)(void *context);   /* NEW, may be NULL */
    rte_status_t (*execute)(void *context);        /* unchanged */
    rte_status_t (*post_execute)(void *context);  /* NEW, may be NULL */
...
Per-cycle order inside `rte_appmanager_run()`'s loop:
...
`rte_appmanager_config_t` gains one new optional field:
...
typedef struct {
    ...
    const rte_appmanager_checkpoint_config_t *checkpoint; /* NEW, NULL = disabled */
} rte_appmanager_config_t;

typedef struct {
    rte_channel_t *vital_channel;   /* checkpoint target */
    rte_duration_ms_t    max_delay_ms;    /* forwarded to rte_checkpoint_config_t */
    uint32_t              expected_node_count;
    rte_watchdog_t       watchdog;        /* optional, may be NULL */
} rte_appmanager_checkpoint_config_t;
```

(ADR-025 superseded the checkpoint target's type: `vital_channel`
became `voter`, of type `rte_voter_t *` - a voter with its channels
...
When `config->checkpoint != NULL`, `rte_appmanager_run()` builds a
...
`rte_channel_checkpoint()` has already called
`rte_safestate_enter(RTE_SAFESTATE_LEVEL_SAFE, RTE_SAFESTATE_REASON_CHECKPOINT_TIMEOUT)`
itself (REQ-CHECKPOINT-003) before returning `RTE_STATUS_TIMEOUT`, and
`rte_appmanager_run()` handles that `RTE_STATUS_TIMEOUT` exactly like
...
framework's shipped `rte_safestate.c`, `RTE_SAFESTATE_LEVEL_SAFE` is an
unconditional, permanent halt (REQ-COMMON-SAFESTATE-002): after invoking
any registered handler, `rte_safestate_enter()` always spins forever
regardless of whether that handler returns, so `rte_channel_checkpoint()`
returning `RTE_STATUS_TIMEOUT` to its caller at all is not reachable
through a normal handler return — the only way is a handler that itself
diverts control flow (e.g. `longjmp`, as `tests/checkpoint/test_rte_checkpoint.c`
already does to verify the SAFE-entry without hanging the test suite).
`rte_appmanager_run()`'s handling of that return value exists so the
...
the `rte_safestate_enter()` call before `rte_channel_checkpoint()` ever
returns.

### 2.3 No change to `rte_checkpoint` or `rte_channel`

This ADR adds no new fields or behavior to either module (ADR-017). This
is deliberately a composition, not a modification: `rte_appmanager`
becomes a caller of `rte_channel_checkpoint()`, the same way any
...
still call `rte_channel_checkpoint()` directly from its own
...
- Positive: a dual-channel application's main-loop boilerplate shrinks —
  ...
  motivating example).
- Positive: reuses `iteration_count`, already-tested `rte_checkpoint`,
  and the existing error/threshold accounting — no new counters, no new
  error-handling path, no new safe-state trigger.
- Negative / accepted cost: `rte_appmanager_config_t` and
  `rte_appmanager_operations_t` grow by one and two pointer-sized fields
...
- Negative / scope limit: checkpoint hook ordering is fixed
  ...
  `rte_channel_checkpoint()` itself from `pre_execute()` and leave
...
### 5.1 `rte_channel_init()`'s `channel_count >= 2` floor

`rte_channel_checkpoint()` requires a `rte_channel_t`, and that
...
Decision (user-directed): relax `rte_channel_init()`'s floor to
`channel_count >= 1`, reachable only via `RTE_VOTING_NMR` with
...
Even with 5.1, `rte_channel_checkpoint()`'s wire format
(`rte_vital_message_t`, ~272 bytes) does not fit A/B's existing peer
link, which was opened with a fixed 16-byte `AB_SAMPLE` frame size
(`rte_netlink`'s `message_size` is fixed per link at `rte_netlink_open()`
time — every message on that link must be exactly that size), and a
background task already exclusively calls `rte_netlink_receive()` on
...
reading alone: `rte_channel_checkpoint()` sends its own arrival marker
and then blocks waiting for an explicit reply confirming that *same*
`checkpoint_id` — nothing in `rte_checkpoint` or in this retrofit's first
version ever generated that reply, so both A and B hung permanently after
their first cycle (`rte_channel_checkpoint()`'s `RTE_STATUS_TIMEOUT`
...
third, independent hazard: `rte_netlink`'s POSIX backend `send()` is a
...
guards every `rte_netlink_send()` call site on that link.

### 5.3 Safety-policy reconciliation, not a framework change

`rte_channel_checkpoint()`'s mandatory SAFE-halt on timeout
...
`rte_channel_checkpoint(NULL, ...)` returns the harmless, recoverable
`RTE_STATUS_INVALID_PARAM` (per 2.2, handled like any other failed
stage) rather than ever reaching the halt-triggering branch. This also
uncovered and fixed a real ordering bug in `rte_appmanager_run()` itself
