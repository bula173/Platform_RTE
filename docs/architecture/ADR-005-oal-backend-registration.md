...
with the framework itself (always returning `SAPI_STATUS_NOT_IMPLEMENTED`),
...
same pattern already used for `sapi_safestate`'s per-level handlers
...
`static const sapi_timer_backend_t my_posix_timer_backend = { ... };` and
calling `sapi_timer_register_backend(&my_posix_timer_backend)` during
system startup, before any other `sapi_timer_*` call. This is the answer
...
One global slot per service (a single `static const sapi_*_backend_t
...
`sapi_safestate`. Registering again replaces the previous backend (no
...
- `SAPI_STATUS_NOT_INITIALIZED`: no backend has been registered for this
  service yet. Previously unused by the OAL services (only by
  handle-lifecycle checks); now also means "you forgot to call
  `sapi_<service>_register_backend()` at startup."
- `SAPI_STATUS_NOT_SUPPORTED`: a backend **is** registered, but the
  specific vtable slot for this operation is `NULL` - the backend
  implements the service but not this particular call.
- `SAPI_STATUS_NOT_IMPLEMENTED`: no longer produced by any OAL service
  (it was the placeholder stub's blanket answer). The value stays in
  `sapi_status_t` (status codes are append-only, ADR-001 section 3.6) but
...
framework, exactly like `sapi_safestate`'s handler array.

### 2.5 `sapi_log` and `sapi_reboot`

`sapi_log` gets the same treatment for consistency even though its default
...
`sapi_reboot` (new in ADR-004 section 3) is built with this pattern from
...
- Negative: every existing OAL test that asserted `SAPI_STATUS_NOT_IMPLEMENTED`
  for valid-parameter cases now asserts `SAPI_STATUS_NOT_INITIALIZED`
...
> per-feature header (`include/safeapi/<feature>/sapi_<feature>.h`) and
> `.c` file (`src/<feature>/sapi_<feature>.c`), not a shared `os/` folder.

Every `include/safeapi/os/*.h` gains a `sapi_<service>_backend_t` type and
a `sapi_<service>_register_backend()` declaration; every
`src/os/sapi_*.c` is rewritten to validate-then-dispatch instead of always
returning `SAPI_STATUS_NOT_IMPLEMENTED`.
