...
Accepted - design of record. Lands **before** ADR-036's train-session
multiplexing, so the Train gateway service is the clean sole owner of
per-train demux when that work begins.
...
- `gateway_c_il_handler` - ILSim -> A/B relay (+ TrainPositionInRoute
  back), and hosts `gateway_c_logctl` (the runtime log-level fan-out)
...
channel-down-reboot boilerplate.

The integrator wants role C built **from several services** rather than
one app - IL, CTC and Train as separate binaries - as a precursor to the
multi-train work and to a planned future **inter-service communication**
...
- `libgateway_c_common.a` (`gateway_c::common`) - the boilerplate lifted
  ...
  parse + `app_main_common_init()`), `gateway_c_common_make_config()`
...
`gateway_c_service_def_t` literal - `{ name, handler_init, handler_execute,
handler_shutdown, handler_check_link_down, pre_handlers }` - and calls
...
The deployment unit stays **one container per site**: `c-west` /
`c-east` are unchanged as Docker service / container names. Inside, a
shipped `/app/bin/gateway_c_entrypoint.sh <SITE>` launches the three
...
- Docker fault-injection (`docker restart c-west`,
  `docker network disconnect ... c-west`) restarts / partitions all
  three services together - the existing `robot/fault_injection/`
  suite needs no change.
- `docker logs c-west` interleaves all three - `get_container_log("c-west")`
  is unchanged in Docker mode.

### 3. Local test env: three processes, one merged per-site log

`setupLocalTestEnv.sh` launches 6 gateway processes (3 per site) instead
of 2, each `ts_launch.py`-wrapped into its own
`dist/local-test-env/logs/c_<svc>_<site>.log`.
`ContainerSupportFunctions.get_container_log("c-west")` in local mode
merges the three `c_*_west.log` files by timestamp, so robot assertions
that scrape `c-west`'s log keep working unchanged.
`ContainerLogAttachListener._ALL_SERVICES` gains
`c-train-west`/`c-il-west`/`c-ctc-west` (+ east) for the `--stream-logs`
live view (per-service visibility); the per-test merged `.txt` continues
to attribute them under those names.
...
- N=2 `rbc_scenario` suite unchanged: **16/16**, 0 A/B SAFE-states, both
  local and `docker compose`.
- `robot/containers/` + `robot/fault_injection/` unchanged and passing
  (container name `c-west`/`c-east` preserved).
- A restart of one gateway service process fails the `c-west` container
  (entrypoint exits non-zero) - confirmed by killing one service in a
  running container.
...
- More processes at runtime (6 gateway processes vs 2) but the **same
  container count** - the deployment and the robot suite's view of "the
