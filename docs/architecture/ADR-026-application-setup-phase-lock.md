...
Status: Accepted
Date: 2026-08-17
Applies to: `include/safeapi/lifecycle/sapi_lifecycle.h` (new),
`src/lifecycle/sapi_lifecycle.c` (new), `sapi_appmanager.h`/`.c`,
`sapi_timer.c`, `sapi_channel.c`, `sapi_voter.c`,
`sapi_cross_comparator.c`, `sapi_watchdog.c`, `sapi_status.h`/`.c`.
...
1. **Single entry point.** `sapi_appmanager_run()` was already documented
   as "the single entry point for all applications" (`sapi_appmanager.h`),
   ...
   silently race on `sapi_appmanager.c`'s own file-scope globals
...
2. **INIT phase vs. RUN phase, locked after start.** A safety-critical
   ...
   constructor (`sapi_timer_create()`, `sapi_channel_init()`, etc.) was
...
3. **No threads: how do channels/dual-channels get serviced?** Raised as
   ...
   this framework's threading model (`sapi_task` remains available and
...
### 2.1 A new, minimal `sapi_lifecycle` module (REQ-LIFECYCLE-001/002)

A single process-wide flag (`include/safeapi/lifecycle/sapi_lifecycle.h`,
implemented in `src/lifecycle/sapi_lifecycle.c`, compiled unconditionally
...
own admission criteria): `sapi_lifecycle_lock()`/`_unlock()`/
`_is_locked()`, plus `sapi_lifecycle_check_setup_allowed()` returning
`SAPI_STATUS_INVALID_STATE` (new status code, appended at value 12 -
`sapi_status.h`'s own ABI-stability comment requires appending, never
renumbering) once locked.

`sapi_appmanager_run()` is the sole caller of `lock()`/`unlock()`:
- `unlock()` at the very top of every call (existing convention: mirrors
  ...
  execution phase ends (entering `SAPI_APP_STATE_SHUTTING_DOWN`) - not
  ...
  two `sapi_appmanager_run()` calls (this framework's own test suite
...
- `lock()` the moment `ops->init()` returns `SAPI_STATUS_OK` (state
  transitions to `SAPI_APP_STATE_RUNNING`).

### 2.2 Which constructors are gated (REQ-LIFECYCLE-001)

Gated (each calls `sapi_lifecycle_check_setup_allowed()` as one of its
...
**Deliberately excluded**: `sapi_netlink_open()`, `sapi_dual_channel_init()`,
`sapi_dual_negotiator_init()`. All three are legitimately re-invoked
...
project, built immediately before this ADR) all call `sapi_netlink_open()`
- and the latter also `sapi_dual_channel_init()`/`sapi_dual_negotiator_init()`
...
`sapi_appmanager_run()` now checks `g_app_state.state` **before** doing
...
framework's own test suite (`tests/appmanager/test_sapi_appmanager.c`)
already relies on calling `sapi_appmanager_run()` sequentially, more than
once, in one process, and continues to after this ADR.

### 2.4 The `longjmp()` escape hatch: `sapi_appmanager_reset_state()`

One existing test (`test_checkpoint_timeout_enters_safestate_before_pre_execute`)
verifies `SAPI_SAFESTATE_LEVEL_SAFE`'s own documented never-returns
...
`sapi_appmanager_run()` skips its own SHUTDOWN phase entirely, leaving
`g_app_state`/the new setup lock stuck at `RUNNING`/locked forever - which
then caused every subsequent test's own `sapi_appmanager_run()` call to
...
path, `sapi_appmanager_reset_state()` was added: forcibly resets
...
`sapi_appmanager_run()` call is genuinely still executing (not abandoned
via a non-local jump) corrupts that call's own state.

## 3. Consequences

**Positive:**
- A concurrent/accidental re-entrant `sapi_appmanager_run()` call is now
...
- The netlink/dual exclusion (2.2) means this lock does **not** fully
  prevent a *misuse* of `sapi_netlink_open()`/`sapi_dual_channel_init()`/
  `sapi_dual_negotiator_init()` to add a genuinely new link mid-run
...
- `sapi_appmanager_reset_state()` is a real, if narrow, addition to the
...
- The setup lock is a single, process-wide flag (REQ-LIFECYCLE-002), not
  one per `sapi_appmanager_config_t` - consistent with `g_app_state`'s
...
- `ctest --test-dir build`: 26/26 passing, including the pre-existing
  `test_sapi_appmanager` sequential-multi-run suite and the `longjmp()`
  safestate test (now paired with `sapi_appmanager_reset_state()`).
...
- `include/safeapi/lifecycle/sapi_lifecycle.h`, `src/lifecycle/sapi_lifecycle.c` (new)
- `include/safeapi/appmanager/sapi_appmanager.h`, `src/appmanager/sapi_appmanager.c`
- `include/safeapi/status/sapi_status.h`, `src/status/sapi_status.c`
- `src/timer/sapi_timer.c`, `src/channel_link/sapi_channel.c`,
  `src/voter/sapi_voter.c`, `src/cross_comparator/sapi_cross_comparator.c`,
