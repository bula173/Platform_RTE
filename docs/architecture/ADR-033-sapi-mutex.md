# ADR-033: rte_mutex - a portable mutual-exclusion primitive
...
`rte_netlink`, ...) already follows is that swapping the platform/RTOS
...
same gap-class ADR-031 (`rte_mem_util`, for `<string.h>`) and ADR-026/032
(`rte_lifecycle`, for setup-phase gating) already closed this project's
history.
...
- `rte_mutex_create()` is setup-phase-gated (a mutex is created once at
  init, same posture as `rte_timer_create()`/`rte_task_create()`);
...
inside `rte_task_destroy()`'s own doc-comment cross-reference is a
