...
Status: Draft
Date: 2026-08-02
Applies to: safeAPIFreamwork, `include/safeapi/common/sapi_safestate.h`,
`include/safeapi/os/sapi_reboot.h`
...
- `SAPI_ASSERT`, `SAPI_SAFESTATE`, `SAPI_REBOOT` - that every layer (L0
...
{
    SAPI_SAFESTATE_LEVEL_DEGRADED = 0,  /* continue, with reduced functionality */
    SAPI_SAFESTATE_LEVEL_SAFE     = 1,  /* fail-safe restrictive state; does not return */
    SAPI_SAFESTATE_LEVEL_REBOOT   = 2   /* SAFE is insufficient; controlled restart; does not return */
...
`sapi_safestate_reason_t` is `uint16_t`. Values 0-4095 are reserved for the
framework itself (`sapi_safestate_reason.h`-style constants such as
`SAPI_SAFESTATE_REASON_ASSERT_FAILED`); application/RBC-specific reason
...
### 2.3 Why `sapi_safestate` does not call `sapi_reboot` directly

`SAPI_REBOOT` is sugar for entering `SAPI_SAFESTATE_LEVEL_REBOOT`. The
...
OAL service, `sapi_reboot.h` (section 3). `src/common` has no OS dependency
(see ADR-002, section 4) and this ADR does not change that: `sapi_safestate.c`
never calls into `src/os`. Instead, the integrator's `REBOOT`-level handler
is expected to call `sapi_reboot_request()` itself:

```c
static void my_reboot_handler(sapi_safestate_level_t level,
                               sapi_safestate_reason_t reason,
                               const char *file, int32_t line,
                               const char *message)
{
    /* safe the outputs, persist a diagnostic record, then: */
    (void)sapi_reboot_request(reason);
    /* falls through to sapi_safestate's own defensive halt if the backend
...
`REBOOT` handlers are documented as must-not-return, but `sapi_safestate_enter`
...
2. If the handler returns anyway (a bug, or no handler was registered at
   all), and the level is `SAFE` or `REBOOT`, `sapi_safestate_enter` enters
...
missing `DEGRADED` handler simply means `sapi_safestate_enter` returns
immediately after recording nothing, which is acceptable since `DEGRADED`
is defined as non-terminal.

### 2.5 `SAPI_ASSERT` is always active

`SAPI_ASSERT(cond)` compiles into every build, including production - it is
...
#define SAPI_ASSERT(cond) \
    do { if (!(cond)) { sapi_safestate_enter(SAPI_SAFESTATE_LEVEL_SAFE, \
        SAPI_SAFESTATE_REASON_ASSERT_FAILED, __FILE__, (int32_t)__LINE__, #cond); } } while (0)
...
Function-like macros here only forward to a real function (`sapi_safestate_enter`)
...
`SAPI_STATUS_NOT_IMPLEMENTED` like every other OAL stub in this codebase.
...
- Negative: because `SAPI_ASSERT` is always active, it has a permanent
...
- Deferred: the OAL backend-selection mechanism referenced in section 2.3
  (how an integrator supplies a real `sapi_reboot_request` - and every
...
> physical layout is `include/safeapi/safestate/sapi_safestate.h` +
> `src/safestate/sapi_safestate.c` (target `safeapi::safestate`) and
> `include/safeapi/reboot/sapi_reboot.h` + `src/reboot/sapi_reboot.c`
> (target `safeapi::reboot`).

`include/safeapi/common/sapi_safestate.h` + `src/common/sapi_safestate.c`
(added to `safeapi_common`, see ADR-002/ADR-003).
`include/safeapi/os/sapi_reboot.h` + `src/os/sapi_reboot.c` (added to
