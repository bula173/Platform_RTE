...
`tests/CMakeLists.txt`, `cmake/SafeAPIHelpers.cmake`,
`cmake/safeAPIFrameworkConfig.cmake.in`, `src/appmanager/CMakeLists.txt`.

## 1. Context

ADR-023 collapsed 22 per-feature CMake targets into 4 grouped static
libraries (`safeapi_core`/`oal`/`channels`/`appmanager`) because no
...
Add one `option(SAFEAPI_ENABLE_<NAME> ... ON)` per toggleable module to
...
`cmake/SafeAPIHelpers.cmake` gains `safeapi_require_feature(<FEATURE>
DEPENDS <dep1> <dep2> ...)`: if `<FEATURE>` is `ON` and any `<depN>` is
`OFF`, configure fails with `FATAL_ERROR` naming exactly which
`-DSAFEAPI_ENABLE_<dep>=ON` to pass. This was chosen over silently
...
in section 2.1 has a matching `safeapi_require_feature()` call in the
top-level `CMakeLists.txt`.

### 2.3 What changes mechanically

- Each grouped library's `add_library(... STATIC ...)` source list is
  now built up conditionally (`if(SAFEAPI_ENABLE_X) list(APPEND ...)
  ...
  (`safeapi_oal`/`safeapi_channels` cannot be an empty library) rather
  than producing a broken zero-source target.
- `tests/CMakeLists.txt`: each optional module's `safeapi_add_test()`
...
- `src/appmanager/CMakeLists.txt`'s inclusion becomes conditional on
  `SAFEAPI_ENABLE_APPMANAGER` (previously gated only on the
  subdirectory's `CMakeLists.txt` existing at all).
- A configure-time summary (`message(STATUS "SAFEAPI_ENABLE_<X>:
  <value>")` per option) so `cmake -B build` output shows the full
  selection without needing `cmake -L`.
- `install(TARGETS ...)` only lists `safeapi_appmanager` when
  `SAFEAPI_ENABLE_APPMANAGER` actually created that target
  (`if(TARGET safeapi_appmanager)`).

### 2.4 What does *not* change

- The 4 library/alias names (`safeapi_core`/`oal`/`channels`/
  `appmanager`, `safeapi::core` etc.) from ADR-023 are unchanged - this
...
- Positive: an integrator can now genuinely exclude a module from
  their build (`-DSAFEAPI_ENABLE_WATCHDOG=OFF -DSAFEAPI_ENABLE_REBOOT=OFF
...
- Negative: 15 new CMake options is more surface for a downstream
  ...
  defaulting `ON` (opt-out, not opt-in) and by `safeapi_require_feature()`
...
- `cmake/SafeAPIHelpers.cmake` - `safeapi_require_feature()`.
- `cmake/safeAPIFrameworkConfig.cmake.in` - unaffected by which options
  were used to build the installed package, beyond `safeapi::appmanager`
