...
Status: Draft
Date: 2026-08-05
Applies to: `rte_checkpoint` (new), `rte_clocksync` (new), and how both
plug into the existing `rte_channel` / `rte_checksum` / `rte_ipc`
/ `rte_watchdog` modules.

## 1. Context

The vital channels that vote on a result (`rte_channel`) can run on
...
- `docs/REDUNDANCY_ARCHITECTURE.md` already specifies a
  `rte_channel_checkpoint()` / `rte_checkpoint_config_t` API (checkpoint
...
- ADR-001's layered diagram names an empty "L1 Safety Communication Layer
  (future ADR)" slot between the OAL (`rte_ipc`/`rte_timer`) and the
  application layer (`rte_channel` lives here); this ADR is that
  future ADR.
- ADR-008 (`rte_channel`, a local two-channel comparator written before
  `rte_channel`'s fuller 2oo2/2oo3/NMR implementation was known
  ...
  `rte_ipc` in a future ADR." This is that ADR. `rte_checkpoint` and
  `rte_clocksync` are designed to plug into `rte_channel` (the
  more complete, actively-developed implementation), not `rte_channel`.

## 2. Decision

### 2.1 Reuse the EN 50159 envelope that already exists — don't rebuild it

`rte_checksum.h`'s `rte_vital_message_t` + `rte_checksum_vital_message_create()`
/ `rte_checksum_vital_message_verify()` already implement most of what an
...
`rte_checkpoint` (below) uses this wire format directly for the
checkpoint-arrival handshake instead of inventing a second envelope
format — the earlier plan to add a standalone `rte_synclink` module is
dropped for exactly this reason.

### 2.2 `rte_checkpoint`: bounded rendezvous, no new transport backend

`rte_checkpoint` adds **no new backend of its own**. It operates on an
already-initialized `rte_channel_t *` and reuses that instance's
...
for voting, e.g. `rte_ipc` over POSIX/RTOS/a real network link, is what
...
```c
rte_status_t rte_channel_checkpoint(rte_channel_t *handle,
                                       const rte_checkpoint_config_t *config);
```

`rte_checkpoint_config_t` keeps the field names already used in
...
(`rte_watchdog_t`, may be NULL) that gets kicked on a successful
checkpoint.

Algorithm: broadcast a small checkpoint-arrival message (`checkpoint_id`
as both the `rte_vital_message_t` sequence number and payload) to every
channel via the vital channel's own `backend_send`, then
`backend_recv`-poll each channel bounded by `max_delay_ms`, verifying
each reply with `rte_checksum_vital_message_verify()` and discarding any
...
`RTE_STATUS_OK`. Otherwise: call
`rte_safestate_enter(RTE_SAFESTATE_LEVEL_SAFE, ...)` directly (the same
pattern `rte_channel_receive()` already uses on a voting
disagreement — see its own doc comment) and return
`RTE_STATUS_TIMEOUT`. `RTE_WATCHDOG_CHECKPOINT` now has real behavior
...
- Positive: no new envelope format, no new transport backend — both new
  ...
  (`rte_checksum`, `rte_ipc`'s backend registration, `rte_watchdog`,
  `rte_safestate`).
...
- Negative / scope limit: `rte_clocksync` provides no protocol
...
- Negative / accepted risk: `rte_checksum`'s `sender_id` is not
  ...
  needed.
- Deferred: `rte_checkpoint`'s relationship to `rte_channel` (ADR-008)
  is not resolved by this ADR — `rte_channel` remains a working, simpler
  same-build two-channel comparator; whether it should be deprecated in
  favor of `rte_channel` + `rte_checkpoint` is a separate decision
