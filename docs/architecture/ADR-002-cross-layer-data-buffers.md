...
Status: Draft
Date: 2026-08-02
Applies to: safeAPIFreamwork, `include/safeapi/common/sapi_buffer.h`
...
raw `(pointer, size)` pairs around (`sapi_nvm_read/write`, `sapi_ipc_send/
...
their own integrity mechanism, e.g. `sapi_nvm`'s CRC-checked reads).
...
(`sapi_mem_pool` acquire/release per buffer) was considered and rejected for
...
A separate read-only view, `sapi_const_buffer_t`, is provided so a producer
...
(`sapi_nvm` already CRC-checks on read; a future L1 layer will define
...
`sapi_buffer_copy_in`/`copy_out` return `SAPI_STATUS_RESOURCE_EXHAUSTED`
...
- Negative / deferred: existing services (`sapi_nvm`, `sapi_ipc`) are **not**
  retrofitted to use `sapi_buffer_t` in this change — they keep their raw
  `(pointer, size)` signatures for now to avoid an unscoped breaking change.
  Adopting `sapi_buffer_t` in those APIs is a follow-up decision once this
...
> physical layout is `include/safeapi/buffer/sapi_buffer.h` +
> `src/buffer/sapi_buffer.c`, target `safeapi::buffer`.

`include/safeapi/common/sapi_buffer.h` + `src/common/sapi_buffer.c`, built
