...
| REQ-OAL-LOG-015 | `sapi_log_set_level(min_level)` shall set a process-global minimum severity, below which `sapi_log_write()` and `sapi_log_write_event()` drop the call before dispatching to the backend (and before any formatting work); `sapi_log_get_level()` shall return the current value. The default is `SAPI_LOG_LEVEL_DEBUG` (nothing filtered). It is callable at any time from any thread (not setup-only); a `min_level` outside `SAPI_LOG_LEVEL_DEBUG..SAPI_LOG_LEVEL_ERROR` leaves the threshold unchanged. A concurrent change can only cause one in-flight best-effort log line to be kept or dropped unexpectedly (REQ-OAL-LOG-001), never a torn read. |
| REQ-OAL-LOG-016 | `sapi_log_level_from_string(name, out_level)` shall parse exactly the four canonical spellings `sapi_log_level_to_string()` renders (`"DEBUG"`/`"INFO"`/`"WARNING"`/`"ERROR"`, case-sensitive) into `*out_level` and return `SAPI_STATUS_OK`; any other `name`, or a NULL `name`/`out_level`, shall return `SAPI_STATUS_INVALID_PARAM` and leave `*out_level` unchanged. |
| REQ-OAL-LOG-017 | A `sapi_log_fields_t` builder shall let a caller accumulate space-separated `key=value` pairs — typed via `sapi_log_fields_add_str/_u32/_i32/_u64/_i64/_hex_u32/_bool()` — for use as `sapi_log_write_event()`'s `extra_fields` (or `info`) argument without hand-rolling `sapi_string` chains and without a variadic (MISRA C:2012 Rule 17.1). It shall use only in-struct storage (no allocation), place exactly one separating space between pairs and none at the ends, truncate rather than reject over-long content (REQ-OAL-LOG-001), tolerate a NULL builder / NULL key at every entry point as a silent no-op, and expose the accumulated text as a never-NULL C string via `sapi_log_fields_c_str()` (`""` when empty). `sapi_log_write_event_fields(...)` shall behave as `sapi_log_write_event(...)` with the builder's text as `extra_fields`, an empty or NULL builder being identical to a NULL `extra_fields`. `sapi_log_write_event()` shall treat an empty-string `extra_fields` identically to NULL (no field, no trailing space). |
