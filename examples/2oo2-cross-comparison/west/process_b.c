 ...
 * @brief Process B - Cross-Comparison Architecture
 *
 * Process B mirrors Process A:
 * 1. Computes independently
 * 2. Receives Process A's command
 * 3. Performs same 2oo2 comparison logic
 * 4. Outputs result
 * 5. Verifies A reached same decision
 *
 * Both A and B execute identical logic, should reach identical decisions
 */
...
int main(int argc, char *argv[])
{
    ...
    fprintf(stderr,
            "┌────────────────────────────────────────────────────────┐\n");
    fprintf(stderr,
            "│ Process B - Cross-Comparison (Peer-to-Peer 2oo2)       │\n");
    fprintf(stderr,
            "│ Mirrors Process A, independent comparison              │\n");
    fprintf(stderr,
            "└────────────────────────────────────────────────────────┘\n");
    fprintf(stderr, "\n");
    ...
    while (1) {
        ...
        for (uint32_t i = 0; i < TRAIN_COUNT; i++) {
            ...
            __sync_synchronize();
            shared_mem.b_ready = 1;

            if (cycle_count % 10 == 0) {
                fprintf(stderr,
                        "[WEST:B] Cycle %u: Computed train=%u, pos=%u, sig=%u, speed=%u\n",
                        cycle_count, cmd.train_id, cmd.position,
                        cmd.signal_state, cmd.speed_limit);
            }
        }

        /* PHASE 2: Wait for A's computation */
        uint32_t timeout_ms = 0;
        while (!shared_mem.a_ready && timeout_ms < 25) {
            usleep(1000);
            timeout_ms++;
        }

        if (!shared_mem.a_ready) {
            fprintf(stderr, "[WEST:B] ERROR: Process A timeout\n");
            continue;
        }

        /* PHASE 3: Cross-comparison (identical logic to A) */
        train_command_t output;
        uint8_t match_status;
        ...
        shared_mem.cmd_b_out = output;
        __sync_synchronize();
        shared_mem.b_compared = 1;

        if (cycle_count % 10 == 0) {
            fprintf(stderr,
                    "[WEST:B] Received A's: train=%u, sig=%u, speed=%u\n",
                    shared_mem.cmd_a.train_id,
            ...
            fprintf(stderr,
                    "[WEST:B] Output: sig=%u, speed=%u\n",
                    output.signal_state, output.speed_limit);
        }

        /* PHASE 4: Verify A reached same decision */
        timeout_ms = 0;
        while (!shared_mem.a_compared && timeout_ms < 10) {
            usleep(1000);
            timeout_ms++;
        }

        if (shared_mem.a_compared) {
            if (shared_mem.cmd_a_out.signal_state == shared_mem.cmd_b_out.signal_state &&
                ...
                fprintf(stderr,
                        "[WEST:B] ⚠️  ALERT: A's output differs!\n");
                fprintf(stderr,
        ...
        shared_mem.b_ready = 0;
        shared_mem.b_compared = 0;
